"""H024 Phase 4 demo adapter-use readiness human decision.

This records a human decision approving adapter-use readiness review only.
It does not approve adapter use, broker request construction, MT5 execution,
...
SCHEMA = "h024_phase4_demo_adapter_use_readiness_human_decision_v1"
KIND = "PHASE4_DEMO_ADAPTER_USE_READINESS_HUMAN_DECISION"
STATUS = "ADAPTER_USE_READINESS_REVIEW_APPROVED_NO_EXECUTION_AUTHORITY"
DECISION = "APPROVE_ADAPTER_USE_READINESS_REVIEW_NO_EXECUTION"

PACKET_SCHEMA = "h024_phase4_demo_adapter_use_readiness_packet_v1"
PACKET_KIND = "PHASE4_DEMO_ADAPTER_USE_READINESS_PACKET_REVIEW_ONLY"
PACKET_DECISION = "REQUEST_HUMAN_ADAPTER_USE_REVIEW_NO_EXECUTION_AUTHORITY"
...
REQUIRED_TRUE_FLAGS = (
    ...
    "adapter_use_readiness_packet_passed",
...
def _require_packet(packet: dict[str, Any]) -> list[str]:
    ...
    if packet.get("schema") != PACKET_SCHEMA:
        violations.append("adapter_use_readiness_packet: schema mismatch")
    if packet.get("kind") != PACKET_KIND:
        violations.append("adapter_use_readiness_packet: kind mismatch")
    if packet.get("decision") != PACKET_DECISION:
        violations.append("adapter_use_readiness_packet: decision mismatch")
    if not _verdict_pass(packet):
        violations.append("adapter_use_readiness_packet: verdict is not PASS")
    if not _violations_zero(packet.get("violations")):
        violations.append("adapter_use_readiness_packet: violations are non-zero")
    return violations


def build_adapter_use_readiness_human_decision_record(
    *,
    adapter_use_readiness_packet: dict[str, Any],
    allowed_demo_server: str | None = None,
) -> dict[str, Any]:
    violations = _require_packet(adapter_use_readiness_packet)

    if allowed_demo_server and not _json_contains_text(adapter_use_readiness_packet, allowed_demo_server):
    ...
    record: dict[str, Any] = {
        ...
        "decision_scope": {
            "adapter_use_readiness_packet_passed": True,
            "approved_scope": "adapter_use_readiness_review_only",
        ...
        "upstream_artifacts": {
            "adapter_use_readiness_packet": _upstream_summary(adapter_use_readiness_packet),
        },
    }

    verification = verify_adapter_use_readiness_human_decision_record(
        record,
        allowed_demo_server=allowed_demo_server,
        require_pass=False,
    )
...
def build_adapter_use_readiness_human_decision_from_files(
    *,
    adapter_use_readiness_packet_jsonl: str | Path,
    output_jsonl: str | Path,
    allowed_demo_server: str | None = None,
) -> dict[str, Any]:
    packet = read_single_jsonl_record(adapter_use_readiness_packet_jsonl)
...
def verify_adapter_use_readiness_human_decision_record(
    record: dict[str, Any],
    *,
    allowed_demo_server: str | None = None,
    ...
    if not isinstance(decision_scope, dict):
        violations.append("decision_scope block missing")
    elif decision_scope.get("approved_scope") != "adapter_use_readiness_review_only":
        violations.append("approved_scope must be adapter_use_readiness_review_only")

    upstream = record.get("upstream_artifacts")
    if not isinstance(upstream, dict):
        violations.append("upstream_artifacts block missing")
    else:
        packet = upstream.get("adapter_use_readiness_packet")
        if not isinstance(packet, dict) or packet.get("decision") != PACKET_DECISION:
            violations.append("adapter_use_readiness_packet upstream summary mismatch")

    return VerificationResult(ok=not violations, violations=tuple(violations))


def verify_adapter_use_readiness_human_decision_file(
    path: str | Path,
    *,
    allowed_demo_server: str | None = None,
    require_pass: bool = False,
) -> VerificationResult:
    return verify_adapter_use_readiness_human_decision_record(
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    build.add_argument("adapter_use_readiness_packet_jsonl")
