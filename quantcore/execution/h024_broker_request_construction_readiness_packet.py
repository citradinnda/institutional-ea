"""H024 broker-request construction readiness packet.

This is a review-only governance packet. It aggregates the approved no-op
adapter-use invocation audit and adapter boundary static verifier, then requests
human review for broker-request construction readiness.

It does not construct broker requests, MT5 requests, order payloads, dispatch
transport, mutate terminal state, mutate broker state, place demo/live orders,
...
SCHEMA = "h024_broker_request_construction_readiness_packet_v1"
KIND = "BROKER_REQUEST_CONSTRUCTION_READINESS_PACKET_REVIEW_ONLY"
STATUS = "READY_FOR_HUMAN_BROKER_REQUEST_CONSTRUCTION_REVIEW_NO_EXECUTION"
DECISION = "REQUEST_HUMAN_BROKER_REQUEST_CONSTRUCTION_REVIEW_NO_EXECUTION_AUTHORITY"

NOOP_INVOCATION_SCHEMA = "h024_demo_adapter_noop_use_invocation_audit_v1"
NOOP_INVOCATION_KIND = "DEMO_ADAPTER_NOOP_USE_INVOCATION_AUDIT"
NOOP_INVOCATION_DECISION = "INVOKE_NOOP_ADAPTER_USE_ONLY_REFUSE_BROKER_TRANSPORT"
...
REQUIRED_TRUE_FLAGS = (
    ...
    "noop_adapter_use_invocation_audit_passed",
    "adapter_boundary_static_verifier_passed",
    "broker_request_construction_readiness_packet_defined",
)
...
def build_broker_request_construction_readiness_packet_record(
    *,
    noop_use_invocation_audit: dict[str, Any],
    ...
    if allowed_demo_server and not _json_contains_text(noop_use_invocation_audit, allowed_demo_server):
    ...
    record: dict[str, Any] = {
        ...
        "readiness": {
            "noop_adapter_use_invocation_audit_passed": True,
            "adapter_boundary_static_verifier_passed": True,
            "broker_request_construction_readiness_packet_defined": True,
            "requested_human_review": "broker_request_construction_readiness_only",
        },
        "required_next_capability_constraints": {
            "must_consume_verified_intent": True,
            "must_not_reinterpret_h020_sizing": True,
            "must_attach_idempotency_key": True,
            "must_require_kill_switch_allow_state": True,
            "must_emit_preview_only_json": True,
            "must_not_import_metatrader5": True,
            "must_not_call_mt5_execution_api": True,
            "must_not_dispatch_transport": True,
        },
        "mutation_safety": {
        ...
        "upstream_artifacts": {
            "noop_use_invocation_audit": _upstream_summary(noop_use_invocation_audit),
    ...
    verification = verify_broker_request_construction_readiness_packet_record(
        record,
        allowed_demo_server=allowed_demo_server,
        require_pass=False,
    )
...
def build_broker_request_construction_readiness_packet_from_files(
    *,
    noop_use_invocation_audit_jsonl: str | Path,
    boundary_static_verifier_jsonl: str | Path,
    output_jsonl: str | Path,
    allowed_demo_server: str | None = None,
) -> dict[str, Any]:
    noop_use_invocation_audit = read_single_jsonl_record(noop_use_invocation_audit_jsonl)
    ...
    record = build_broker_request_construction_readiness_packet_record(
        noop_use_invocation_audit=noop_use_invocation_audit,
...
def verify_broker_request_construction_readiness_packet_record(
    record: dict[str, Any],
    *,
    allowed_demo_server: str | None = None,
    ...
    elif readiness.get("requested_human_review") != "broker_request_construction_readiness_only":
        violations.append("requested_human_review must be broker_request_construction_readiness_only")

    constraints = record.get("required_next_capability_constraints")
    if not isinstance(constraints, dict):
        violations.append("required_next_capability_constraints block missing")
    else:
    ...
    if not isinstance(upstream, dict):
        ...
        invocation = upstream.get("noop_use_invocation_audit")
        boundary = upstream.get("boundary_static_verifier")
        if not isinstance(invocation, dict) or invocation.get("decision") != NOOP_INVOCATION_DECISION:
            violations.append("noop_use_invocation_audit upstream summary mismatch")
        if not isinstance(boundary, dict) or boundary.get("kind") != BOUNDARY_KIND:
            violations.append("boundary_static_verifier upstream summary mismatch")

    return VerificationResult(ok=not violations, violations=tuple(violations))


def verify_broker_request_construction_readiness_packet_file(
    path: str | Path,
    *,
    allowed_demo_server: str | None = None,
    require_pass: bool = False,
) -> VerificationResult:
    return verify_broker_request_construction_readiness_packet_record(
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    build.add_argument("noop_use_invocation_audit_jsonl")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "build":
        record = build_broker_request_construction_readiness_packet_from_files(
            noop_use_invocation_audit_jsonl=args.noop_use_invocation_audit_jsonl,
