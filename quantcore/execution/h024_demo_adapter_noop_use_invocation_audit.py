"""H024 demo adapter no-op use invocation audit.

This audit exercises the explicitly approved pure-Python no-op adapter-use path.
...
SCHEMA = "h024_demo_adapter_noop_use_invocation_audit_v1"
KIND = "DEMO_ADAPTER_NOOP_USE_INVOCATION_AUDIT"
STATUS = "NOOP_ADAPTER_USE_INVOKED_REFUSED_BROKER_TRANSPORT"
DECISION = "INVOKE_NOOP_ADAPTER_USE_ONLY_REFUSE_BROKER_TRANSPORT"

NOOP_USE_APPROVAL_SCHEMA = "h024_demo_adapter_noop_use_approval_v1"
NOOP_USE_APPROVAL_KIND = "DEMO_ADAPTER_NOOP_USE_APPROVAL"
NOOP_USE_APPROVAL_DECISION = "APPROVE_NOOP_ADAPTER_USE_ONLY_NO_BROKER_REQUEST_AUTHORITY"
...
def build_noop_use_invocation_audit_record(
    *,
    noop_use_approval: dict[str, Any],
    ...
    if allowed_demo_server and not (
        _json_contains_text(noop_use_approval, allowed_demo_server)
        or _json_contains_text(noop_transport_contract, allowed_demo_server)
    ):
    ...
    record: dict[str, Any] = {
        ...
        "invocation": {
            "noop_transport_contract_available": True,
            "noop_adapter_use_invoked": True,
            "noop_transport_contract_invoked": True,
            "broker_transport_refused": True,
            "request_construction_refused": True,
            "invocation_result": "NOOP_REFUSAL_NO_BROKER_TRANSPORT",
        },
        "mutation_safety": {
        ...
        "upstream_artifacts": {
            "noop_use_approval": _upstream_summary(noop_use_approval),
    ...
    verification = verify_noop_use_invocation_audit_record(
        record,
        allowed_demo_server=allowed_demo_server,
        require_pass=False,
    )
...
def build_noop_use_invocation_audit_from_files(
    *,
    noop_use_approval_jsonl: str | Path,
    noop_transport_contract_jsonl: str | Path,
    output_jsonl: str | Path,
    allowed_demo_server: str | None = None,
) -> dict[str, Any]:
    noop_use_approval = read_single_jsonl_record(noop_use_approval_jsonl)
    ...
    record = build_noop_use_invocation_audit_record(
        noop_use_approval=noop_use_approval,
...
def verify_noop_use_invocation_audit_record(
    record: dict[str, Any],
    *,
    allowed_demo_server: str | None = None,
    ...
    if not isinstance(invocation, dict):
        violations.append("invocation block missing")
    elif invocation.get("invocation_result") != "NOOP_REFUSAL_NO_BROKER_TRANSPORT":
        violations.append("invocation_result must be NOOP_REFUSAL_NO_BROKER_TRANSPORT")

    upstream = record.get("upstream_artifacts")
    if not isinstance(upstream, dict):
        violations.append("upstream_artifacts block missing")
    else:
        noop_use = upstream.get("noop_use_approval")
        noop_transport = upstream.get("noop_transport_contract")
        if not isinstance(noop_use, dict) or noop_use.get("decision") != NOOP_USE_APPROVAL_DECISION:
            violations.append("noop_use_approval upstream summary mismatch")
        if not isinstance(noop_transport, dict) or noop_transport.get("decision") != NOOP_TRANSPORT_DECISION:
            violations.append("noop_transport_contract upstream summary mismatch")

    return VerificationResult(ok=not violations, violations=tuple(violations))


def verify_noop_use_invocation_audit_file(
    path: str | Path,
    *,
    allowed_demo_server: str | None = None,
    require_pass: bool = False,
) -> VerificationResult:
    return verify_noop_use_invocation_audit_record(
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    build.add_argument("noop_use_approval_jsonl")
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = verify_noop_use_invocation_audit_file(
        args.input_jsonl,
        allowed_demo_server=args.allowed_demo_server,
