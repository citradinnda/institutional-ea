# HANDOFF 22 - Superseding Update After HistData M1 Inventory
...
This HANDOFF_22 file is intentionally structured like prior handoffs:

1. This top section is the newest superseding update.
2. The complete HANDOFF_21 content is included below this update so the next AI has the full prior project context.
3. If any older detail below conflicts with this HANDOFF_22 update, the HANDOFF_22 update wins.
4. Do not ignore the older full context; it contains the project rules, history, strategy conventions, repo hygiene lessons, and do-not rules.

## Current Verified State At HANDOFF_22
...
Latest verified commit before this handoff document is expected to include:

- `Record HistData M1 acquisition inventory`

Latest verified handoff before this update:
...
- Current correct full-test anchor is `494 passed`.
- If tests pass but the count drops below `494` without a deliberate test-removal phase, treat it as a regression.
...
Expected status after this HANDOFF_22 document is committed and pushed:
...
## Major Update Since HANDOFF_21

The project path changed because the user successfully downloaded approximately five years of M1 data from HistData.
...
## Phase 3.24 Completed After HANDOFF_21
...
- `C:\Users\equin\Documents\institutional-ea\HANDOFF_22.md`

Purpose:

1. Pivot from Dukascopy-only evaluation to HistData inventory.
2. Preserve the distinction between HistData and Dukascopy.
...
## HistData Raw Files Inventoried
...
- Do not rename or move files until that action is explicitly planned and documented.
...
The files appear to use a no-header HistData-style CSV format:
...
This differs from the observed Dukascopy format:
...
- Do not use the Dukascopy loader as the official source loader for HistData.
- A dedicated HistData loader should be designed and tested.
...
This may solve the infrastructure blocker that broker-native MT5 M1 history was too short.

However, the long date range alone does not make HistData research-grade.

HistData must go through explicit loader validation, provenance documentation, and coverage checks before any H017 validation can use it.
...
1. Inspect existing loader APIs before writing code.
2. Design a dedicated HistData loader rather than pretending HistData is Dukascopy.
3. Add focused tests for the observed no-header format.
4. Parse timestamps carefully.
5. Keep timezone assumptions explicit.
6. Reject duplicate timestamps before canonicalization.
7. Reject non-monotonic timestamps before canonicalization.
8. Validate OHLC and volume.
9. Report missing minutes without automatically failing.
10. Preserve source identity as HistData.

Before writing code, inspect actual APIs with:

    inspect.signature(...)
    dataclasses.fields(...)

Do not trust remembered function names.

## Absolute Do-Not Rules At HANDOFF_22
...
8. Do not accept HistData as a research source until loader validation, provenance, and coverage checks are complete.
...
Understood. I am continuing after Phase 3.24 and HANDOFF_22.

I understand:

1. Windows / PowerShell / VS Code / Python 3.12.10 in `.venv`.
2. Current full-test anchor is `494 passed`.
3. Phase 3.24 inventoried new HistData M1 files for USDJPY and XAUUSD.
4. The original HistData files were preserved exactly as downloaded.
5. The HistData files are under `C:\Users\equin\Documents\institutional-ea\data\raw\dukascopy_samples`, which is currently a misleading folder name.
6. The HistData raw format appears to be no-header `YYYY.MM.DD,HH:MM,Open,High,Low,Close,Volume`.
7. HistData is not accepted as a research source yet.
8. H017 is alive but not promotable.
9. Do not run H017 validation on HistData yet.
10. The next logical sub-phase is Phase 3.25: inspect HistData raw format and design a dedicated tested HistData loader.
11. First task is hygiene verification only. No new code yet.
...
# Complete Prior Context From HANDOFF_21 Follows
# HANDOFF 21 - Superseding Update After Phase 3.23

You are continuing an existing project. Read this entire prompt before responding. Do not invent context. When in doubt, ask before writing code.

This HANDOFF_21 file is intentionally structured like HANDOFF_20:

1. This top section is the newest superseding update.
2. The complete HANDOFF_20 content is included below this update so the next AI has the full prior project context.
3. If any older detail below conflicts with this HANDOFF_21 update, the HANDOFF_21 update wins.
4. Do not ignore the older full context; it contains the project rules, history, strategy conventions, repo hygiene lessons, and do-not rules.

## Current Verified State At HANDOFF_21
...
Latest verified project commit before this handoff document:

- `de27e02 Add Dukascopy coverage check plan`

Recent verified log context:
...
- `2eb65fa Add handoff document #19 after Dukascopy sample inspection`
...
Latest verified push result for project commit:

- `ef57b8c..de27e02 main -> main`

Latest verified repo status after Phase 3.23 push:
...
Latest verified full test output before this handoff:

- `494 passed in 14.07s`
...
## Expected State After This HANDOFF_21 Is Committed

Expected latest commit after this handoff is committed:

- `Add handoff document #21 after Dukascopy coverage plan`

Expected latest project commit before the handoff commit:
...
Expected status after this HANDOFF_21 document is committed and pushed:
...
## Phase 3.22 Completed After HANDOFF_20

Phase 3.22 name:

- Run tested Dukascopy loader on existing tiny local samples and document loader-based inspection
...
1. Run `load_dukascopy_csv` on the two existing local Dukascopy sample files.
2. Confirm loader output matches the previous manual/mechanical inspection.
3. Document the loader-based result.
4. Preserve non-acceptance of Dukascopy.
5. Preserve rule that sample data is not H017 validation evidence.

Raw local sample files checked:
...
These raw files are under `/data/`, are gitignored, and remain uncommitted.
...
- first missing minutes begin at:
...
Phase 3.22 interpretation:

1. The tested Dukascopy loader reproduces the expected tiny-sample results.
2. This improves infrastructure confidence only.
3. It does not improve H017 research confidence.
4. Dukascopy remains under evaluation only.
5. Dukascopy is still not accepted as a research source.

## Phase 3.23 Completed After HANDOFF_20

Phase 3.23 name:

- Create Dukascopy coverage-check plan
...
- Define what a controlled small multi-day Dukascopy coverage check must measure before any larger export or H017 validation use.
- Keep this as documentation/planning only.
- Avoid bulk data acquisition before measurement criteria are written down.
- Preserve rule that Dukascopy is not yet accepted.

This phase did not:

1. Download bulk data.
2. Commit raw data.
3. Run H017 validation on Dukascopy.
4. Tune H017.
5. Accept Dukascopy as a research source.

Recommended first multi-day window in the plan:

- `2024-01-02 through 2024-01-08 UTC`

Reason:

1. Short enough to inspect quickly.
2. Includes several normal weekdays.
3. Includes a weekend boundary.
4. Close to the already inspected `2024-01-03` tiny samples.
5. Helps distinguish normal market closures from suspicious gaps.

Recommended local raw-data directory for future coverage-check files:

- `C:\Users\equin\Documents\institutional-ea\data\raw\dukascopy_coverage_check`

This directory is under `/data/`, which is gitignored.

Required measurements from the plan:
...
Required cross-day checks from the plan:
...
Phase 3.23 interpretation:

1. Passing the future small multi-day coverage check would not fully accept Dukascopy.
2. It would only allow a later larger coverage/provenance phase.
3. Dukascopy remains a candidate, not an accepted research source.

## Current Dukascopy Status At HANDOFF_21

Dukascopy is still not accepted as a research source.

The loader exists only so controlled inspection can continue.

Do not use Dukascopy data as H017 validation evidence yet.

Do not combine Dukascopy M1 with Exness H4 silently.

Do not tune H017 using Dukascopy data.

Do not commit raw Dukascopy CSV files.

## Current H017 Status At HANDOFF_21
...
This should be operational only.

Purpose:

1. Download or export only the small planned multi-day window.
2. Use only USDJPY and XAUUSD.
3. Save raw files under the gitignored `/data/` directory.
4. Document provenance.
5. Stop before analysis if anything is unclear.

Recommended planned window from Phase 3.23:
...
The next AI should not assume it can directly download the data.

The user may need to manually operate the Dukascopy/JForex/history-export interface.

A good Phase 3.24 response should first ask for hygiene verification, then guide the user through a small controlled download/export process and provenance recording.

Phase 3.24 should record:

1. Data source name.
2. Data source interface used.
3. Download date.
4. Exact symbol names exported.
5. Timeframe.
6. Timezone setting.
7. Date range requested.
8. Exported filenames.
9. Any manual settings used.
10. Any source warnings or limitations.

## Absolute Do-Not Rules At HANDOFF_21
...
3. Do not tune H017.
4. Do not change the cost model.
5. Do not commit raw CSV files.
6. Do not change `.gitignore` from `/data/` to `data/`.
7. Do not start Phase 4 execution code.
8. Do not start live trading.
9. Do not treat the short-window 2026 event result as validated edge.
10. Do not ignore the `-33.65%` drawdown.
11. Do not broaden to more symbols yet.
12. Do not add machine learning yet.
13. Do not accept any external source without loader-validation tests and documented provenance/coverage.
14. Do not continue development while local commits are unpushed.
15. Do not let `git status` go unread.
16. Do not write code that calls internal APIs before inspecting actual signatures.
17. Do not use Linux/macOS shell syntax in PowerShell.
18. Do not skip test-count verification.

## Exact First Response The Next AI Should Give

The next AI should respond briefly:

Understood. I am continuing after Phase 3.23 and HANDOFF_21.

I understand:

1. Windows / PowerShell / VS Code / Python 3.12.10 in `.venv`.
2. Latest verified project commit before this handoff is `de27e02 Add Dukascopy coverage check plan`.
3. Current full-test anchor is `494 passed`.
4. Phase 3.20 added the tested Dukascopy CSV loader.
5. Phase 3.22 confirmed the loader reproduces the tiny local USDJPY and XAUUSD sample results.
6. Phase 3.23 added `docs/operations/DUKASCOPY_COVERAGE_CHECK_PLAN.md`.
7. Dukascopy is still not accepted as a research source.
8. H017 is alive but not promotable.
9. The realistic event pipeline works, but broker-native M1 coverage is too short for research-grade validation.
10. The next logical sub-phase is Phase 3.24: acquire a tiny multi-day Dukascopy coverage sample under the gitignored `/data/` directory.
11. First task is hygiene verification only. No new code yet.
...
# Complete Prior Context From HANDOFF_20 Follows
# HANDOFF 20 - Superseding Update After Phase 3.20

You are continuing an existing project. Read this entire prompt before responding. Do not invent context. When in doubt, ask before writing code.

This HANDOFF_20 file is intentionally self-contained.

Important structure note:

1. This top section is the newest superseding update.
2. The complete HANDOFF_19 content is included below this update so the next AI has the full prior project context.
3. If any older detail below conflicts with this HANDOFF_20 update, the HANDOFF_20 update wins.
4. Do not ignore the older full context; it contains the project rules, history, strategy conventions, repo hygiene lessons, and do-not rules.

## Current Verified State At HANDOFF_20
...
Latest verified code commit before this handoff document:
...
Latest verified push result for code commit:

- `a0a3085..5a2bf46 main -> main`

Latest verified repo status after Phase 3.20 code push:
...
Important test-count update:

- Previous anchor from HANDOFF_19 was `482 passed`.
- Phase 3.20 deliberately added 12 Dukascopy loader tests.
- New correct anchor is `494 passed`.
- If tests pass but the count drops below `494` without a deliberate test-removal phase, treat it as a regression.

Latest verified focused test output:

- `12 passed in 1.53s`

Latest verified full test output:

- `494 passed in 11.69s`
...
Expected status after this HANDOFF_20 document is committed and pushed:
...
- `Add handoff document #20 after Dukascopy loader`

Expected latest code commit before the handoff commit:
...
## Phase 3.19 Completed After HANDOFF_19

Phase 3.19 name:

- Inspect existing loader APIs before designing a Dukascopy loader
...
- `C:\Users\equin\Documents\institutional-ea\docs\operations\DUKASCOPY_LOADER_API_INSPECTION.md`

Purpose:

- Inspect actual repository APIs before designing the Dukascopy loader.
- Avoid relying on stale handoff memory or guessed function names.
- Preserve the discipline of inspecting real signatures before calling internal functions.
...
1. Generic canonical loader utilities live in:

   - `C:\Users\equin\Documents\institutional-ea\quantcore\data\loaders.py`

2. MT5-specific loader lives in:

   - `C:\Users\equin\Documents\institutional-ea\quantcore\data\mt5_loader.py`

3. Important correction to HANDOFF_19 memory:
...
7. Existing `ensure_canonical(df)` behavior:

   - lowercases columns
   - validates required canonical columns
   - localizes naive timestamp indexes to UTC
   - converts aware timestamp indexes to UTC
   - drops duplicate timestamps, keeping first
   - sorts ascending

8. Important future-loader warning:

   - `ensure_canonical` silently drops duplicate timestamps.
   - A vendor-acceptance loader should not hide bad vendor rows.
   - Therefore, the Dukascopy loader should reject duplicates before calling `ensure_canonical`.

## Phase 3.20 Completed After HANDOFF_19

Phase 3.20 name:

- Add tested Dukascopy CSV loader
...
- Add a tested loader for observed Dukascopy CSV samples.
- Convert Dukascopy CSV rows into the project’s canonical OHLCV format.
- Add validation around schema, timestamps, OHLC integrity, prices, volume, missing minutes, and immutable metadata.
...
Actual `DukascopyLoadResult` fields:
...
Supported observed Dukascopy CSV schema:
...
Dukascopy loader behavior:

1. Reads comma-separated CSV files.
2. Requires columns:
   - `UTC`
   - `Open`
   - `High`
...
4. Rejects empty CSV data.
5. Parses timestamps using the observed UTC timestamp format.
6. Produces a UTC timezone-aware pandas `DatetimeIndex`.
7. Rejects duplicate timestamps before canonicalization.
8. Rejects non-monotonic timestamps before canonicalization.
9. Converts OHLCV columns to numeric.
10. Rejects non-numeric OHLCV values.
11. Rejects non-positive OHLC prices.
12. Rejects structurally bad OHLC rows.
13. Rejects negative volume.
14. Allows zero volume.
15. Reports missing minutes between first and last timestamp.
16. Does not automatically fail on missing minutes.
17. Calls `ensure_canonical`.
18. Returns frozen `DukascopyLoadResult`.

Validation order note:

- The loader checks non-positive OHLC prices before checking bad OHLC structure.
- This was necessary because a row with `open = 0` is both non-positive and structurally bad.
- The more specific error should be `non-positive OHLC prices`.

Dukascopy tests added:

1. `test_load_dukascopy_csv_returns_result`
2. `test_load_dukascopy_csv_outputs_canonical_columns_and_utc_index`
3. `test_load_dukascopy_csv_metadata_matches_data`
4. `test_load_dukascopy_csv_rejects_missing_required_column`
5. `test_load_dukascopy_csv_rejects_duplicate_timestamps`
6. `test_load_dukascopy_csv_rejects_non_monotonic_timestamps`
7. `test_load_dukascopy_csv_rejects_bad_ohlc_rows`
8. `test_load_dukascopy_csv_rejects_non_positive_prices`
9. `test_load_dukascopy_csv_rejects_negative_volume`
10. `test_load_dukascopy_csv_allows_zero_volume`
11. `test_load_dukascopy_csv_reports_missing_minutes_without_failing`
12. `test_dukascopy_load_result_is_frozen`

## Current Dukascopy Status
...
Raw Dukascopy sample files remain local and gitignored:

- `C:\Users\equin\Documents\institutional-ea\data\raw\dukascopy_samples\USD-JPY_Minute_2024-01-03_UTC.csv`
- `C:\Users\equin\Documents\institutional-ea\data\raw\dukascopy_samples\XAU-USD_Minute_2024-01-03_UTC.csv`

## Dukascopy Sample Findings From Phase 3.18
...
If this handoff document is treated as Phase 3.21, the next development phase should be:

### Phase 3.22 - Run tested Dukascopy loader on existing tiny local samples and document loader-based inspection
...
2. Confirm loader output matches the previous manual/mechanical inspection.
3. Document the loader-based result.
4. Do not accept Dukascopy yet.
5. Do not use sample data as H017 validation evidence.

Suggested first command after hygiene:
    ...
    paths = [
        Path(r"C:\Users\equin\Documents\institutional-ea\data\raw\dukascopy_samples\USD-JPY_Minute_2024-01-03_UTC.csv"),
        Path(r"C:\Users\equin\Documents\institutional-ea\data\raw\dukascopy_samples\XAU-USD_Minute_2024-01-03_UTC.csv"),
    ]

    for path in paths:
        print(f"\n{path}")
        result = load_dukascopy_csv(path)
        ...
        print("first_missing_minutes:", result.missing_minutes[:10])
...
Expected loader-based sample output:
...
- latest_utc=2024-01-03 23:59:00+00:00
- n_missing_minutes=60
- first missing minutes begin at 2024-01-03 22:00:00+00:00

Suggested documentation file if confirmed:

- `C:\Users\equin\Documents\institutional-ea\docs\operations\DUKASCOPY_LOADER_SAMPLE_CHECK.md`

Do not commit raw CSV files.

## Strategic Interpretation At HANDOFF_20

Current institutional conclusion:

1. The realistic event-driven pipeline works.
2. H017 remains alive but not promotable.
3. Broker-native MT5 M1 history is too short for research-grade validation.
4. External M1 source evaluation is now underway.
5. Dukascopy is the first external M1 candidate.
6. Tiny Dukascopy sample files were manually and mechanically inspected.
7. A tested Dukascopy CSV loader now exists.
8. Dukascopy is still not accepted as a research source.
9. Next work should validate the tested loader against the existing local samples and document the result.
10. No live trading.
11. No Phase 4 execution code yet.
12. No tuning H017 to short-window or vendor-specific quirks.

## Absolute Do-Not Rules At HANDOFF_20
...
Understood. I am continuing after Phase 3.20 and HANDOFF_20.

I understand:

1. Windows / PowerShell / VS Code / Python 3.12.10 in `.venv`.
2. Latest verified code commit before this handoff is `5a2bf46 Add tested Dukascopy CSV loader`.
3. Current full-test anchor is `494 passed`.
4. Phase 3.19 documented actual loader APIs in `docs/operations/DUKASCOPY_LOADER_API_INSPECTION.md`.
5. Phase 3.20 added `quantcore/data/dukascopy_loader.py` and `tests/test_dukascopy_loader.py`.
6. The Dukascopy loader has 12 focused tests.
7. Dukascopy is still not accepted as a research source.
8. H017 is alive but not promotable.
9. The realistic event pipeline works, but broker-native M1 coverage is too short for research-grade validation.
10. The next logical development step is to run the tested Dukascopy loader on the existing tiny local samples and document the loader-based sample check.
11. First task is hygiene verification only. No new code yet.
...
# Complete Prior Context From HANDOFF_19 Follows
