"""Real-data diagnostic for H018 stop-state / held-signal mismatch."""
...
def main() -> None:
    print("H018 stop-state diagnostic")
    print("=" * 60)
    ...
    diagnostic = diagnose_h018_stop_state(
    ...
    print(f"total_nonzero_signal_count={diagnostic.total_nonzero_signal_count}")
    print(f"flat_signal_skipped_count={diagnostic.flat_signal_skipped_count}")
    print(f"nan_signal_skipped_count={diagnostic.nan_signal_skipped_count}")
    print(
        "unavailable_price_or_stop_skipped_count="
        f"{diagnostic.unavailable_price_or_stop_skipped_count}"
    )
    print(
        "same_side_stop_valid_at_decision_close_count="
        f"{diagnostic.same_side_stop_valid_at_decision_close_count}"
    )
    print(
        "same_side_stop_breached_at_decision_close_count="
        f"{diagnostic.same_side_stop_breached_at_decision_close_count}"
    )
    print(f"fresh_signal_count={diagnostic.fresh_signal_count}")
    print(f"held_continuation_count={diagnostic.held_continuation_count}")
    print(f"breached_fresh_signal_count={diagnostic.breached_fresh_signal_count}")
    print(
        "breached_held_continuation_count="
        f"{diagnostic.breached_held_continuation_count}"
    )
    print(
        "bucket_counts_by_symbol_side="
        f"{diagnostic.bucket_counts_by_symbol_side}"
    )
    print()

    print("First breached same-side stop observations")
    print("-" * 40)
    breached = [
        obs
        for obs in diagnostic.observations
        if not obs.same_side_stop_valid_at_decision_close
    ]
    for obs in breached[:20]:
        print(
            f"symbol={obs.symbol}, "
            f"side={obs.side}, "
            f"decision_time={obs.decision_time}, "
            f"entry_time={obs.entry_time}, "
            f"signal_value={obs.signal_value}, "
            f"previous_signal_value={obs.previous_signal_value}, "
            f"signal_state={obs.signal_state}, "
            f"decision_close={obs.decision_close}, "
            f"stop_price={obs.stop_price}, "
            f"selected_stop_panel={obs.selected_stop_panel}, "
            "same_side_stop_valid_at_decision_close="
            f"{obs.same_side_stop_valid_at_decision_close}"
        )
    ...
    print(
        "- This counts H017 held Donchian signals whose same-side raw "
        "Chandelier stop is non-protective at decision close."
    )
    print(
        "- A breached held-continuation count supports the stale-held-signal "
        "/ stop-lifecycle mismatch diagnosis."
    )
    print("- This diagnostic does not switch stop panels.")
