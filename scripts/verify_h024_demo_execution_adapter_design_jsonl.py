...
from quantcore.execution.h024_demo_execution_adapter_design import verify_h024_demo_execution_adapter_design_record
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify H024 demo-only execution adapter design JSONL.")
    parser.add_argument("demo_execution_adapter_design_jsonl", type=Path)
    ...
    records = _read_jsonl(args.demo_execution_adapter_design_jsonl)

    violations: list[str] = []
    if args.require_design and not records:
        violations.append("missing_required_demo_execution_adapter_design_record")
    ...
    print(f"Input demo execution adapter design JSONL: {args.demo_execution_adapter_design_jsonl}")
