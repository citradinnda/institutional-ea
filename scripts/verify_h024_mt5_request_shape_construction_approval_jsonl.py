...
EXPECTED_SCHEMA = "h024_mt5_request_shape_construction_approval_v1"
EXPECTED_KIND = "MT5_REQUEST_SHAPE_CONSTRUCTION_APPROVAL"
EXPECTED_STATUS = "MT5_REQUEST_SHAPE_PREVIEW_CONSTRUCTION_APPROVED_NO_REQUEST_NO_DISPATCH"
EXPECTED_DECISION = "APPROVE_INERT_MT5_REQUEST_SHAPE_PREVIEW_ONLY_NO_REQUEST_NO_DISPATCH"
...
def verify_record(record: Mapping[str, Any], allowed_demo_server: str | None, require_approved: bool) -> list[str]:
    ...
    if nested_bool(record, "approved_scope", "may_construct_inert_mt5_request_shape_preview_envelope") is not True:
        violations.append("missing_preview_construction_scope")
    ...
    if require_approved and nested_bool(record, "approved_scope", "may_construct_inert_mt5_request_shape_preview_envelope") is not True:
        violations.append("required_approved_preview_scope_missing")
...
def main() -> int:
    ...
    print(f"H024 MT5 request-shape construction approval records: {len(records)}")
