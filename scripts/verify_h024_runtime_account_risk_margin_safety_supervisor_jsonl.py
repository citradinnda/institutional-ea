...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Verify the H024 runtime account risk/margin safety supervisor JSONL packet.")
    parser.add_argument("path", help="Path to H024 runtime account risk/margin safety supervisor JSONL.")
    parser.add_argument("--require-pass", action="store_true", help="Fail unless every account risk/margin record verdict is PASS.")
    return parser
...
def main() -> int:
    ...
    verification = verify_h024_runtime_account_risk_margin_safety_supervisor_records(records, require_pass=args.require_pass)
    ...
    print(f"H024 runtime account risk/margin safety supervisor records: {verification.get('record_count')}")
