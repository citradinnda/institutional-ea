...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Verify the H024 runtime safety aggregate supervisor JSONL packet.")
    parser.add_argument("path", help="Path to H024 runtime safety aggregate supervisor JSONL.")
    parser.add_argument("--require-pass", action="store_true", help="Fail unless every aggregate record verdict is PASS.")
    return parser
...
def main() -> int:
    ...
    verification = verify_h024_runtime_safety_aggregate_supervisor_records(records, require_pass=args.require_pass)

    embedded_violations = sum(len(record.get("violations", [])) for record in records)
    first_record = records[0] if records else {}

    print(f"H024 runtime safety aggregate supervisor records: {verification.get('record_count')}")
    ...
    if isinstance(first_record, dict):
        for summary in first_record.get("upstream_summaries", []):
            print(
                "Upstream "
