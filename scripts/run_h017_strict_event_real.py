...
@dataclass(frozen=True)
class StrictExpandedValidationResult:
    """Bundle strict expanded H017 validation artifacts for manual inspection."""
...
def _annualized_sharpe(returns: pd.Series, *, periods_per_year: int) -> float:
    """Report a familiar Sharpe scale while leaving promotion logic to H017Claim."""
...
def run_validation() -> StrictExpandedValidationResult:
    """Run the strict expanded broker-native H017 validation path.
    ...
    - preserves native H4 index semantics,
    - passes accepted entry timestamps to the strict event wrapper,
    ...
    return StrictExpandedValidationResult(
...
def _print_strict_execution_summary(result: StrictH017EventBacktestResult) -> None:
    print("Strict event wrapper execution")
    print("-" * 40)
...
def main() -> None:
    ...
    print("H017 strict expanded broker-native event-driven validation")
    print("=" * 60)
    ...
    print("H017 claim on strict expanded broker-native event-driven returns")
    print("-" * 40)
    print(claim.summary)
    print()
    ...
    print("-" * 40)
    print("STRICT BRIDGE-WINDOW PREFLIGHT PASSED: True")
    print(f"H017 PROMOTABLE BY CLAIM: {claim.promotable}")
    print("EXPANDED VALIDATION IS RESEARCH EVIDENCE ONLY: True")
    print("LIVE TRADING APPROVED: False")
    print()
    print("Interpretation guardrails:")
