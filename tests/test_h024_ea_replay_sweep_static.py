...
def test_replay_sweep_uses_override_without_refactoring_execution_path():
...
def test_replay_sweep_emits_state_and_intended_rows_once_per_attach():
    assert 'WritePreflightRow(\n      "H024_REPLAY_SWEEP",' in TEXT
    assert 'WritePreflightRow(\n         "H024_REPLAY_SWEEP_SHIFT",' in TEXT
    assert 'WritePreflightRow(\n      "H024_REPLAY_SWEEP_DONE",' in TEXT
...
def test_tick_and_timer_do_not_repeat_sweep_rows():
