...
Keep /data/ root-anchored in .gitignore.
Do not change .gitignore from /data/ to unanchored data/.
Do not treat smoke success as research sufficiency.
Do not treat short-window profit as validated edge.
Do not start Phase 4 live-execution code until the research blocker is explicitly resolved or deferred.
Preserve the current event-driven timing convention:
...
Preserve the conservative fill rule:
...
Seek another validated data source.
Lower the research threshold with justification.
Defer H017 promotion.
Continue engine hardening without claiming research-grade validation.