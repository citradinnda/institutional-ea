...
If any older handoff conflicts with this file, this HANDOFF_65 wins.
...
Current full-test anchor after latest code work: 777 passed.
If full test count drops below 777 without planned test removal, treat as a regression.
...
777 passed in 16.88s

If full tests pass but count drops below 777 without explicit test-removal intent, stop and treat it as a regression.
...
Build terminal/account preflight logging.
Build a no-order MT5 terminal probe script if needed.
...
Add a terminal/account preflight logger that uses the Python MetaTrader5 API to read runtime facts and write a local CSV/JSON report, still with no order-send capability.

Purpose:

verify terminal connection
account login/server/currency/leverage
trade allowed flags
symbol visibility
symbol selected state
current bid/ask/spread
symbol trade mode/execution/filling/order modes
volume min/max/step
stops/freeze level
timestamp
confirm no order-send function is called
...
This script may import MetaTrader5 to read account/symbol/terminal metadata.
It must not call mt5.order_send.
It must not place, modify, close, or delete orders.
It must be tested with monkeypatch/fake MT5 object so tests do not require a terminal.
User authorization should be requested before running the real terminal preflight.

Alternative next gate:

If avoiding MT5 API work, add a verifier/summary doc for the dry-run action CSV. But the verifier already exists and passed. The more useful next gate is terminal/account preflight logging.
...
allow full-test count to drop below 777 without explicit test-removal intent
...
The next recommended gate is terminal/account preflight logging with no order-send capability.
