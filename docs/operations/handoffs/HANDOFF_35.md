# HANDOFF 35 - Self-Contained Continuation After H017 Raw-Entry Invalid-Stop Guard

This handoff is intentionally self-contained enough for a new AI to continue safely.

If any older handoff conflicts with this file, this HANDOFF_35 wins.
...
Latest expected commit before this HANDOFF_35 document is committed:

    5a53f18 Document H017 invalid stop guard implementation

Expected latest commit after this handoff is committed:

    Add handoff document #35 after H017 invalid stop guard
...
- Current correct full-test anchor is 535 passed.
- If tests pass but the count drops below 535 without a deliberate test-removal phase, treat it as a regression.
...
New raw-entry invalid-stop execution guard:
...
- This guard does not authorize real-data rerun.
- Current focused synthetic tests are in `tests/test_h017_event.py`.

Still undecided:

- executable-entry sizing,
- minimum stop-distance guard,
- maximum notional/leverage guard,
- whether any trade-skipping or clipping policy should open H018.
...
Do not rerun broad strict real-data validation as a promotion attempt after the invalid-stop guard. Any rerun must be explicitly authorized as diagnostics only, and H017 remains failed unless a separate governance phase says otherwise.
...
## Work Completed Since HANDOFF_34

### Commit 9bc8130 - Add H017 execution semantics decision plan
...
- H017 remains failed,
- the fatal 518.77-lot USDJPY event,
- the raw-entry versus executable-entry sizing question,
- directional stop validity questions,
- minimum stop-distance questions,
- maximum notional/leverage questions,
- H017/H018 boundary questions,
- synthetic tests before any real-data reruns.
...
### Commit e854b4a - Add H017 execution semantics decision record

File added:

    docs/operations/H017_EXECUTION_SEMANTICS_DECISION_RECORD.md

The draft decision record documented:

- inspected current H017 event API,
- current raw-entry absolute stop-distance sizing behavior,
- no explicit directional invalid-stop gate before this work,
- no silent fixes,
- synthetic tests before real-data reruns,
- H018 boundary likely required for material trade eligibility/exposure changes.
...
### Commit ef7f717 - Fail closed on invalid H017 event stop geometry
...
- fail-closed validation before sizing,
- long/buy stop must be below raw H4 entry open,
- short/sell stop must be above raw H4 entry open.

Added synthetic tests:

- `test_long_stop_above_raw_entry_fails_closed`
- `test_short_stop_below_raw_entry_fails_closed`
...
### Commit 5a53f18 - Document H017 invalid stop guard implementation
...
- README full-test anchor to 535 passed,
- execution semantics decision record from draft to implemented partial decision record,
- documented the raw-entry directional invalid-stop guard,
- preserved H017 failed/not-promotable status,
- preserved no-live-trading and no-real-data-rerun boundaries.
...
4. Should there be a minimum stop distance relative to spread, ATR, tick size, or all-in friction?
...
Next logical sub-phase after HANDOFF_35 hygiene:

    Phase 3.26-bl - Decide next execution-semantics response
...
2. Read current execution-semantics docs:
   - README.md
   - docs/operations/H017_EXECUTION_SEMANTICS_DECISION_PLAN.md
   - docs/operations/H017_EXECUTION_SEMANTICS_DECISION_RECORD.md
...
3. Decide whether the next work is:
   - synthetic tests for equality invalid stops,
   - synthetic tests for near-zero positive stop distance,
   - an executable-entry sizing decision plan,
   - a maximum leverage/notional guard decision plan,
   - opening H018,
   - or documentation-only ledger updates.

Strong recommendation:

Do not start by changing H017 parameters.

Do not call the invalid-stop guard a fix for H017.

Do not rerun broad real-data validation as if H017 is alive/promotable.

Do not implement trade-skipping or clipping without opening the H018 question.
...
4. Should same-price stops be tested explicitly? Current guard treats equality as invalid:
...
6. Should minimum stop distance be a multiple of spread, ATR, tick size, or all-in friction?
...
## Absolute Do-Not Rules At HANDOFF_35
...
34. Do not allow test count to drop below 535 without explicit test-removal phase.
35. Do not call the raw-entry invalid-stop guard H017 promotion.
36. Do not rerun broad strict validation after the invalid-stop guard unless explicitly authorized as diagnostics only.
37. Do not introduce trade skipping or clipping without opening the H018 boundary question.
...
Understood. I am continuing after HANDOFF_35.

I understand:

1. Windows / PowerShell / VS Code / Python 3.12.10 in `.venv`.
2. Current full-test anchor is 535 passed.
3. Latest expected commit is `Add handoff document #35 after H017 invalid stop guard`.
4. Previous commit is `5a53f18 Document H017 invalid stop guard implementation`.
...
10. The fatal stop geometry was raw H4 entry open `110.770000000` and H017 long stop `110.770240804`.
11. `H017EventInvalidStopError` is now implemented.
12. Under current raw-entry sizing semantics, long/buy stops must be below raw H4 entry open and short/sell stops must be above raw H4 entry open.
13. Invalid directional stop geometry now fails closed.
...
17. Executable-entry sizing remains undecided.
18. Minimum stop-distance guard remains undecided.
19. Maximum notional/leverage guard remains undecided.
20. Do not tune H017.
21. Do not use HistData.
22. Do not broaden symbols or add ML.
23. Do not start Phase 4.
24. Next logical sub-phase is Phase 3.26-bl: decide next execution-semantics response.
25. First task is hygiene verification only. No new code yet.
