...
If any older handoff conflicts with this file, this HANDOFF_66 wins.
...
HANDOFF_65 - After H024 Broker Reconciliation And Dry-Run Log Verification
...
Current full-test anchor after latest code work: 791 passed.
If full test count drops below 791 without planned test removal, treat as a regression.
...
likely one untracked local report remains:
reports/h024_mt5_terminal_preflight.json

That local JSON should not be committed.
...
0e4ef37 Record H024 broker symbol spec audit result
aeef6c2 Add H024 broker symbol spec audit
Current Test Anchor
...
If full tests pass but count drops below 791 without explicit test-removal intent, stop and treat it as a regression.
...
No real-time account/server preflight logging path exists inside EA runtime.
...
Terminal-level trade_allowed was false in the Python preflight and must be understood before later EA/runtime work.

What is currently justified:

Continue dry-run/log-only preparation.
Build a log-only EA skeleton only if it cannot send orders.
Build EA-runtime preflight logging.
Build a terminal-attached no-order runtime probe.
Add verifiers for runtime log shape and safety boundaries.
...
Add a terminal-attached log-only EA/runtime skeleton or equivalent MQL5 probe that cannot send orders and only writes preflight/log records.

Purpose:

verify EA can attach to chart
verify EA runtime symbol/account/terminal facts
verify runtime kill switch defaults to blocked
verify no order-send path exists
verify H024 dry-run intent can be represented as logs only
verify output file shape can be parsed and verified from Python
...
No OrderSend path in log-only EA.
No CTrade execution path in log-only EA.
No Phase 4 approval.

Possible files:

mql5\Experts\H024_LogOnly_Preflight.mq5
...
But first inspect current repo structure for MQL5 conventions before creating anything.

Alternative next gate:

Add a result doc for terminal preflight verifier output if desired.
This is lower value than EA runtime/log-only preflight.
...
allow full-test count to drop below 791 without explicit test-removal intent
...
Understood. Continuing from HANDOFF_66.

I understand:

Windows / PowerShell / VS Code / Python 3.12.10 in .venv.
Branch should be main, clean except possibly local reports\h024_mt5_terminal_preflight.json, and pushed.
Current full-test anchor is 791 passed.
...
Local reports\h024_mt5_terminal_preflight.json must not be committed.
2023 weakness remains real and must not be hidden with filters.
The next recommended gate is a terminal-attached log-only EA/runtime preflight skeleton with no order-send path.
