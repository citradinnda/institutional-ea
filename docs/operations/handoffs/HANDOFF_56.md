# HANDOFF 56 - After H021 Stop Precursor And Candidate Exclusion Diagnostics

If any older handoff conflicts with this file, this HANDOFF_56 wins.
...
- `C:\Users\equin\Documents\institutional-ea\docs\operations\handoffs\HANDOFF_56.md`
...
- Code edit:
  ...
  - Current full-test anchor: `623 passed`.
- If full tests pass but count drops below `623` without explicit planned test removal, stop and treat as regression.
...
Latest commit should be Add handoff document #56 after H021 exclusion diagnostics
Recent previous commits should include:
Add H021 candidate exclusion diagnostic
Add H021 stop precursor diagnostic
...
Current Test Anchor
Current full-test anchor after H021 candidate exclusion diagnostic code:

623 passed in 27.45s
Recent anchors:
...
If full test count drops below 623 without planned test removal, treat it as a regression.
...
HANDOFF_56 is now authoritative and supersedes H53, H54, and H55 where conflicts exist.
Important Paths
Code:
...
docs\operations\handoffs\HANDOFF_56.md
...
Worst time bucket:
decision_hour_utc=05 / entry_hour_utc=09
585 fills
117 stops
20.00% stop rate
-2621.09 PnL
...
H021 evidence so far says bad stop-outs are concentrated, but concentration filters alone do not reveal a profitable core.
...
There is not yet evidence of a promotable positive edge.
Current H021 question:

Is there any sizeable decision-time observable bucket combination with positive expectancy after costs under strict broker-native H4/M1 simulation?
Important:
...
Rank decision-time observable buckets and bucket combinations by within-bucket expectancy.
...
Suggested minimum sample thresholds:
...
mean PnL descending,
only include groups with enough fills.
Interpretation rules:

A profitable bucket is not a strategy.
Tiny buckets are likely noise.
A bucket with PF > 1 and 30 fills is only a lead.
A bucket with PF > 1 and 100+ fills is more interesting, but still not validated.
Any positive-core finding needs later temporal stability checks, not immediate deployment.
Do not:
...
allow full-test count to drop below 623 without explicit test-removal intent.
...
Understood. Continuing after HANDOFF_56.

I understand:

Windows / PowerShell / VS Code / Python 3.12.10 in .venv.
Current branch should be main.
Current full-test anchor is 623 passed.
H017 failed.
...
best retained set was XAUUSD only with PnL -1921.21 and PF 0.835695.
Current H021 question is whether any sizeable decision-time observable bucket combination has positive expectancy after costs.
...
After hygiene passes, I will continue with H021 positive bucket search diagnostics, not strategy implementation.