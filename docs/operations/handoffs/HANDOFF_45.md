HANDOFF 45 - After H018 Portfolio Gross Leverage Policy Acceptance
This handoff is intentionally self-contained enough for a new AI to continue safely.

If any older handoff conflicts with this file, this HANDOFF_45 wins.
...
C:\Users\equin\Documents\institutional-ea\docs\operations\handoffs\HANDOFF_45.md

Latest committed policy commit before this handoff:
...
Add handoff document #45 after H018 portfolio leverage policy acceptance

Expected full-test anchor:

552 passed

Important test-count rule:

If tests pass but the count drops below 552 passed without an explicit test-removal phase, treat it as a regression.
...
Latest commit: Add handoff document #45 after H018 portfolio leverage policy acceptance.
Previous commit: 0bc94a2 Accept H018 portfolio gross leverage policy.
Full tests: 552 passed.
...
docs/operations/handoffs/HANDOFF_45.md
...
Placement in _build_symbol_interval_fill:

Validate raw-entry directional stop geometry.
Validate minimum raw stop distance.
Compute preliminary PositionSize.
If lots are zero, return None.
Validate maximum per-trade USD gross leverage.
Only then build execution-cost-adjusted entry fill.
...
Tests added/repaired in tests/test_h017_event.py:
...
Accepted for implementation, not implemented yet.
...
Raise an explicit H018 portfolio-wide gross leverage violation error.
...
Accepted intended implementation placement:

For each symbol, validate directional stop geometry.
For each symbol, validate minimum raw stop distance.
For each symbol, compute preliminary PositionSize.
For each symbol, validate maximum per-trade USD gross leverage.
Collect all non-zero-lot candidate positions for the interval before creating fills.
Validate maximum portfolio-wide USD gross leverage across collected candidate positions.
Only then build execution-cost-adjusted entry fills for the interval.
The guard must run before any fill is created for a violating interval.

Required future error audit fields:
...
Single-symbol interval below 10.0x passes.
Single-symbol interval exactly 10.0x passes.
Single-symbol interval above 10.0x fails closed.
Two-symbol interval with each trade individually below 10.0x but combined portfolio exposure below 10.0x passes.
Two-symbol interval with each trade individually below or equal to 10.0x but combined portfolio exposure exactly 10.0x passes.
Two-symbol interval with each trade individually below or equal to 10.0x but combined portfolio exposure above 10.0x fails closed.
USDJPY notional is converted from JPY to USD by dividing by entry_raw_price.
XAUUSD notional_quote is treated as USD.
Long and short notionals are summed gross and not netted.
Fail-closed error audit fields are preserved.
Existing per-trade maximum leverage behavior remains unchanged.
Existing minimum stop-distance behavior remains unchanged.
Existing raw-entry invalid-stop behavior remains unchanged.
Full pytest count must not drop below current 552 passed anchor unless explicit test-removal phase exists.
...
Portfolio-wide USD gross leverage guard.
Still not chosen:
...
Current Code Structure Relevant To Next Implementation
In quantcore/backtest/h017_event.py, current interval loop roughly does:

keeps current_equity;
for each decision interval:
sets interval_start_equity;
loops through _SYMBOLS;
calls _build_symbol_interval_fill(...) for each symbol;
appends returned fills;
sums interval P&L;
raises insolvency error if equity <= 0.
Current _build_symbol_interval_fill(...) does too much in one function:
...
Builds cost-adjusted entry fill.
Calls simulate_bracket_trade.
Builds exit cost.
Computes P&L.
Returns Fill.
For portfolio-wide leverage guard, this structure probably needs a two-stage refactor:

Stage A:

Build a preliminary candidate per symbol:
symbol
side
decision_time
entry_time
...
position_size
notional_usd
Apply directional stop guard.
Apply minimum stop-distance guard.
Compute PositionSize.
If zero lots, no candidate.
Apply per-trade leverage guard.
Do not create fills yet.
Stage B:

Collect all non-zero-lot candidates for the interval.
Validate portfolio-wide gross leverage across candidates.
Only after portfolio validation, build execution-cost-adjusted fills.
Do not silently change fill rules, cost model, raw-entry sizing, stop validity, or P&L accounting during this refactor.

Recommended Next Path
After HANDOFF_45 hygiene verification, do not run real-data validation.

Recommended next sub-phase:

Implement H018 portfolio-wide gross leverage guard.

Safe outline:

Read git status.
Run git log --oneline -10.
Run pytest -q and confirm 552 passed.
Inspect APIs before coding:
dataclasses.fields(PositionSize)
dataclasses.fields(InstrumentSpec)
inspect.signature(size_position_from_risk)
relevant signatures in h017_event.py.
Inspect tests/test_h017_event.py before editing because it has a UTF-8 BOM; use utf-8-sig if scripting edits.
Inspect quantcore/backtest/h017_event.py before editing because it also has a UTF-8 BOM; use utf-8-sig if scripting edits.
Add error class:
H018MaximumPortfolioGrossLeverageError
Add constant:
_MAXIMUM_PORTFOLIO_USD_GROSS_LEVERAGE = 10.0
Add portfolio validator:
_validate_maximum_portfolio_usd_gross_leverage(...)
Reuse _position_notional_usd(...) if possible.
Refactor interval fill generation into candidate creation plus fill creation if necessary.
Add focused synthetic tests required by the decision record.
Run focused event tests. Expected count will increase beyond current 30 focused event tests depending on number added.
Run full pytest -q. Full count must be at least 552 + new tests.
Commit and push.
...
Understood. I am continuing after HANDOFF_45.

I understand:

Windows / PowerShell / VS Code / Python 3.12.10 in .venv.
Current expected full-test anchor is 552 passed.
Latest expected policy commit before handoff is 0bc94a2 Accept H018 portfolio gross leverage policy.
Expected handoff commit is Add handoff document #45 after H018 portfolio leverage policy acceptance.
...
H018 portfolio-wide gross leverage policy is accepted for implementation but not yet implemented.
...
Recommended next logical sub-phase is hygiene verification, then implementation of the H018 portfolio-wide gross leverage guard.
