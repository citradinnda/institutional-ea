...
If any older handoff conflicts with this file, this HANDOFF_71 wins.
...
Current full-test anchor after latest code work: 849 passed.
If full test count drops below 849 without planned test removal, treat as a regression.
...
Latest full suite after EA 0.6 strategy-intent code work:

849 passed in 15.94s

If full tests pass but count drops below 849 without explicit test-removal intent, stop and treat it as a regression.
...
Recent Work Since HANDOFF_70
Commit 43bac50 - Add H024 EA state observation parity verifier
...
Parse EA-emitted H024_STATE_OBSERVATION rows.
Convert MT5 server/broker time to UTC.
Map broker symbols:
USDJPYm -> USDJPY
XAUUSDm -> XAUUSD
Compute equivalent Python H024 state for the same closed H4 timestamp.
Use the EA's bounded runtime warmup (h4_warmup_bars) to avoid false mismatches caused by full-history ATR warmup differences.
Compare ints, floats, and booleans with tight tolerance.

Initial synthetic verifier tests passed.

Full suite before commit:

848 passed in 16.79s
Commit 2c817a8 - Record H024 EA state observation parity result

The user updated broker-native H4 exports through at least:
...
EA-emitted H024_STATE_OBSERVATION fields matched Python H024 calculations.
This validates runtime state observation.
It does not validate executable intent, sizing, order placement, or execution adapter behavior.

Result doc:

docs\operations\H024_EA_STATE_OBSERVATION_PARITY_RESULT.md
Commit 6999a46 - Add H024 EA log-only strategy intent rows

Patched EA to version 0.6.
...
Added log-only strategy-derived intent logic:

H024StrategyIntentDetail()
WriteH024StrategyIntentRow()

The EA now emits additional INTENT rows derived from frozen H024 signal semantics:

Possible strategy-derived intent details:

NO_ACTION:strategy_unavailable_insufficient_h4_warmup
NO_ACTION:strategy_no_signal;closed_h4_time=<server time>;mode=log_only_no_execution
WOULD_OPEN:side=long;closed_h4_time=<server time>;source=H024_STATE_OBSERVATION;mode=log_only_no_execution
WOULD_OPEN:side=short;closed_h4_time=<server time>;source=H024_STATE_OBSERVATION;mode=log_only_no_execution
BLOCKED:strategy_conflict_log_only

Important:

This is log-only observation.
No sizing was added.
No order-send was added.
No execution adapter was added.
No MT5 trade API surface was added.

Static source verifier still passed with 0 violations.

Runtime log verifier was updated from EA 0.5 to EA 0.6.

Verifier now accepts constrained WOULD_OPEN rows only if:

kill_switch_blocked == true
detail contains mode=log_only_no_execution
detail includes side=long or side=short

Verifier rejects unconstrained WOULD_OPEN.

Full suite after code work:
...
849 passed
Commit bf7d7c5 - Record H024 EA log-only strategy intent runtime result

EA 0.6 local runtime preflight was run.

First compile/reset run:

Automation target preflight: PASS
Copied EA source to terminal MQL5\Experts
Runtime CSV reset: no existing file
...
Then the user manually attached the EA to the required charts, without attach/detach automation and without order-send.
...
State observation parity verifier:
...
Strategy intent sample:

NO_ACTION:strategy_no_signal;closed_h4_time=2026.05.08 16:00:00;mode=log_only_no_execution

Interpretation:

EA 0.6 passed log-only runtime preflight with strategy-derived intent rows enabled.
EA 0.6 state observation parity still passed.
All observed strategy-derived intent rows were NO_ACTION:strategy_no_signal.
No WOULD_OPEN row was observed in this runtime sample because the observed closed H4 state had no signal.
This does not approve demo/live/Phase 4.
This does not validate WOULD_OPEN behavior under actual signal conditions.
...
EA 0.6 runtime log-only strategy-intent preflight passed with 82 rows, 0 violations.
EA 0.6 state parity passed in that runtime sample with 325 comparisons, 0 violations.
...
Roughly 75-85% of the way to a log-only shadow EA.
...
Validate WOULD_OPEN row semantics without adding execution surface.

Safe options:

Controlled synthetic/log verifier path:
Extend tests or an offline harness to prove constrained WOULD_OPEN details are accepted and unconstrained ones rejected.
This is partly done in the runtime log verifier.
Could add more explicit tests for both long and short, conflict block, and kill-switch enforcement.
Controlled historical/synthetic EA logic harness:
Add a pure function/test-style harness for H024StrategyIntentDetail semantics if feasible without MT5 execution.
This should remain log-only and not touch MT5 order APIs.
Wait for a real log-only runtime signal:
Manually attach EA in log-only mode and collect rows when a real H024 signal occurs.
...
allow full-test count to drop below 849 without explicit test-removal intent
...
Understood. Continuing from HANDOFF_71.

I understand:

Windows / PowerShell / VS Code / Python 3.12.10 in .venv.
Branch should be main, clean except possibly local reports/, and pushed.
Current full-test anchor is 849 passed.
H024 is promising but remains not demo-approved, not live-approved, and not Phase 4-approved.
Latest known commit before this handoff is bf7d7c5 Record H024 EA log-only strategy intent runtime result.
EA runtime input version is 0.6.
MQL5 property version is 0.600.
EA 0.5 state observation parity passed: 16 rows checked, 400 comparisons, 0 violations.
EA 0.6 adds log-only strategy-derived INTENT rows.
EA 0.6 runtime preflight passed: 82 rows, 0 violations.
EA 0.6 runtime state parity passed: 13 rows checked, 325 comparisons, 0 violations.
Observed EA 0.6 strategy intent rows were all NO_ACTION:strategy_no_signal.
...
The next recommended gate is validating constrained WOULD_OPEN row semantics without adding execution surface.
