# HANDOFF 59 - After H021 Signal-Flip and Bridge Hold-Horizon Diagnostics

If any older handoff conflicts with this file, this HANDOFF_59 wins.
...
- Code edit:
  ...
  - Current full-test anchor: `655 passed`.
- If full tests pass but count drops below `655` without explicit planned test removal, stop and treat as regression.
...
Add handoff document #59 after H021 bridge horizon diagnostic
...
Current Test Anchor
Current full-test anchor after H021 bridge hold-horizon diagnostic code:

655 passed in 16.36s
Recent anchors:
...
If full test count drops below 655 without planned test removal, treat it as a regression.
...
Preserve the H021 temporal stability and bridge horizon results in handoffs for now.
...
Phase 4 is not approved.
H021 Earlier Diagnostics Summary
Diagnostic 1 - Trade Decomposition
Script:
...
Diagnostic 2 - Stop-Loss Precursors
Script:
...
Diagnostic 3 - Candidate Exclusions
Script:
...
Diagnostic 4 - Positive Bucket Search
Script:
...
Diagnostic 5 - Positive Bucket Temporal Stability
Script:
...
XAUUSD buy 06: PnL +275.109797, PF 1.258930, median -2.189000
XAUUSD buy 17: PnL +115.232795, PF 1.229937, median -1.207500
USDJPY sell 18: PnL +122.492119, PF 1.190697, median -0.964329
XAUUSD decision_hour_utc 17: PnL +133.556329, PF 1.151079, median +0.493500
XAUUSD decision_hour_utc 06: PnL +197.086724, PF 1.124625, median -0.894000
XAUUSD sell 05: PnL +157.742462, PF 1.121236, median -2.620000
USDJPY buy 01: PnL +80.258698, PF 1.040025, median -0.371867
Interpretation:
...
Mechanical 2, 3, and 4 H4-bar holds remained profitable in aggregate.
...
3 bars: -0.251977
4 bars: -0.574893
This suggests the current strategy may have some directional/entry value.
It also shows lifecycle, stop, and risk shaping are still fragile and unresolved.
This is not deployable evidence.
Do not implement a strategy directly from this diagnostic.
...
H021 has useful structural clues.
...
We may be nearing a better pre-registered lifecycle hypothesis, but not deployment.
Recommended next action:

Preserve this handoff.
Then pause before adding more diagnostics.
If continuing, choose a pre-registered lifecycle diagnostic rather than bucket-mining.
Potential next diagnostic angle:
Compare fixed 1/2/3/4 H4 lifecycle variants as complete portfolio backtests, not only replaying baseline signal_flip fills.
Pre-register pass/fail criteria before running.
Include temporal splits.
Preserve H018 hard guards.
Do not implement a deployable EA from the current evidence.
Deployment Reality Check
The user asked whether we are nearing first demo deployment.

Answer:

No, not yet.
We are closer to understanding why the research has failed.
We do not yet have a validated strategy.
...
allow full-test count to drop below 655 without explicit test-removal intent.
...
Understood. Continuing after HANDOFF_59.

I understand:

Windows / PowerShell / VS Code / Python 3.12.10 in .venv.
Current branch should be main.
Current full-test anchor is 655 passed.
H017 failed.
