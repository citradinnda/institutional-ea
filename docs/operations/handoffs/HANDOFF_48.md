# HANDOFF 48 - After Diagnostic Projected Friction Calculator, Before Explicit H018 Real-Validation Decision

This handoff is intentionally self-contained enough for a new AI to continue safely.

If any older handoff conflicts with this file, this HANDOFF_48 wins.
...
- `C:\Users\equin\Documents\institutional-ea\docs\operations\handoffs\HANDOFF_48.md`
...
- For code changes touching strategy, event engine, sizing, accounting, data loading, or validation logic:
  ...
  - commit,
  - push.
- Current full-test anchor after the latest code commit is `566 passed`.
- If code tests pass but the count drops below `566` without an explicit test-removal phase, treat it as a regression.
...
- Code edit:
  ...
  - Run full `python -m pytest -q`.
  - Current full-test anchor: `566 passed`.
- If full tests pass but count drops below `566` without planned test removal, stop and treat as regression.
...
- `Add handoff document #48 after friction diagnostic calculator`
...
- `566 passed in 15.53s`

Focused friction test result before this handoff:

- `8 passed in 1.88s`

No code changed after that test. The last code work was the diagnostic friction calculator.
...
- Latest commit: `Add handoff document #48 after friction diagnostic calculator`.
- Previous commit: `adb5fab Add diagnostic projected friction calculator`.
...
- `C:\Users\equin\Documents\institutional-ea\tests\test_friction.py`
...
- `docs/operations/handoffs/HANDOFF_48.md`
...
- Diagnostic projected friction calculator in `quantcore/backtest/friction.py`.
...
- No real-data validation rerun unless user explicitly authorizes it after this handoff.
- No H018 claim.
- No live trading.
- No Phase 4 execution.

## Latest Code Addition: Diagnostic Projected Friction Calculator
...
Add a diagnostic-only projected round-trip friction calculator.
...
- stop_slippage_price
- spread_burden_usd
- commission_burden_usd
- stop_slippage_burden_usd
- total_burden_usd

Method:

- `fraction_of_equity(equity_usd: float) -> float`
...
- atr
- optional cost_spec
- optional instrument_spec

Current formula:

- spread burden = one full modeled spread * lots * contract size, converted to USD
- commission burden = two fills * lots * commission per lot per fill
- stop-slippage burden = ATR * stop slippage fraction * lots * contract size, converted to USD
- total burden = spread + commission + modeled stop slippage

USD conversion:

- Uses existing `quote_pnl_to_usd`.
- USDJPY JPY burden is converted to USD by dividing by USDJPY entry price.
- XAUUSD burden is already USD.

Tests:

- XAUUSD uses USD quote directly.
- USDJPY converts JPY burden to USD.
- Non-positive lots, entry price, and ATR are rejected.
- Cost spec symbol mismatch is rejected.
- Instrument spec symbol mismatch is rejected.
- `fraction_of_equity` rejects non-positive equity.
...
The friction calculator is intentionally a standalone primitive. It is not yet integrated into `h017_event.py`.

Future options:

1. Integrate projected friction diagnostics into `_SymbolIntervalCandidate`.
2. Build a diagnostic report over candidate trades.
3. Later, if evidence justifies it, define a fail-closed friction burden cap.
4. Do not choose a threshold casually.

## Strict Expanded Broker-Native Validation Result Before Guards

Script:

- `scripts/run_h017_strict_event_real.py`

Strict bridge-window preflight previously passed.
...
In `quantcore/backtest/h017_event.py`, current interval loop does:
...
## Maintainability Inspection Findings Before This Handoff

Read-only inspection was performed before adding friction calculator.

Findings:

- `h017_event.py` was around 927 lines.
- `test_h017_event.py` was around 1081 lines.
- Candidate/fill split is conceptually sound.
- Guard test grouping is sensible.
- No urgent bug was found.
- Minor style-only issues exist, such as blank-line spacing and function order.
- A defensive `if stop_distance_price <= 0.0: return None` may now be unreachable after guards, but removing it is not worth a standalone change.
- Do not spend time on cosmetic cleanup unless it is bundled with meaningful work.

## Recommended Next Path

The user said they want to create this handoff and “actually start the real deal.”

Do not assume “real deal” means live trading or Phase 4.
...
Reasonable safe interpretation:

- Begin explicit H018 strict validation-preparation work.
- Then run accepted broker-native strict validation only if the user explicitly confirms that is what they mean.

Recommended next steps after this handoff is committed and pushed:

1. Hygiene verification:
   - `git status`
   - `git log --oneline -10`

2. Ask user to explicitly confirm what “real deal” means:
   - strict H018 validation on accepted broker-native USDJPY/XAUUSD M1/H4 data?
   - diagnostic dry-run only?
   - validation plan first?
   - integrate friction diagnostics first?
   - something else?

3. If and only if real-data validation is explicitly authorized:
   - inspect `scripts/run_h017_strict_event_real.py` before running it;
   - confirm it uses accepted complete-window bridge rules;
   - confirm it uses Exness broker-native data only;
   - confirm it does not use HistData;
   - confirm it does not modify or commit raw data;
   - run the intended script;
   - capture complete output;
   - classify failure/pass carefully;
   - do not treat pass/fail as live approval.

4. If the user wants validation plan first:
   - create a short plan in chat, not a big governance doc unless needed.
   - Keep it operational:
     - input files,
     - preflight checks,
     - expected accepted window count,
     - command to run,
     - output artifacts,
...
- Do not allow code-test count to drop below `566` without explicit test-removal phase.
- Do not call implemented guards H018 validation.
- Do not call accepted portfolio leverage policy H018 validation.
- Do not rerun broad strict validation after guards unless explicitly authorized as diagnostics or H018 validation.
...
- Chat paste can corrupt indentation or markdown rendering. Trust git status/tests/file previews over copied chat formatting.

## Exact First Response The Next AI Should Give

The next AI should respond briefly:

Understood. Continuing after HANDOFF_48.

I understand:

- Windows / PowerShell / VS Code / Python 3.12.10 in `.venv`.
- Current branch should be `main`.
- Latest expected commit should be `Add handoff document #48 after friction diagnostic calculator`.
- Previous expected commit should be `adb5fab Add diagnostic projected friction calculator`.
- Current full-test anchor is `566 passed`.
- Docs-only changes do not require full pytest by default.
- Code changes still require focused tests and full `python -m pytest -q`.
...
- No real-data rerun is authorized unless the user explicitly authorizes it.
- Do not tune H017.
- Do not broaden symbols or add ML.
- Implemented guards include invalid-stop, minimum stop-distance, per-trade leverage, and portfolio gross leverage.
- Diagnostic projected friction calculator is implemented but is not a guard and has no threshold.
...
After hygiene passes, I will ask you to explicitly clarify what “real deal” means before running real-data validation, starting Phase 4, or doing anything live-trading-related.