# HANDOFF 51 - After H018 Stop-State Diagnostic Before H019

If any older handoff conflicts with this file, this HANDOFF_51 wins.
...
- `C:\Users\equin\Documents\institutional-ea\docs\operations\handoffs\HANDOFF_51.md`
...
If code tests pass but the count drops below `577` without an explicit test-removal phase, treat it as a regression.
...
- Code edit:
  ...
  - Run full `python -m pytest -q`.
  - Current full-test anchor: `577 passed`.
- If full tests pass but count drops below `577` without planned test removal, stop and treat as regression.
...
- `Add handoff document #51 before H019`
...
- This is not blocking because the tree was clean and pushed.
...
- `577 passed in 14.80s`

Focused test result for latest diagnostic:

- `tests/test_h018_stop_state_diagnostic.py`
- `2 passed in 1.94s`

No code should change after those tests except this docs-only handoff file if it is being created.
...
- Latest commit should be `Add handoff document #51 before H019`
- Previous commit should be `79a464f Add H018 stop-state diagnostic`
...
Strategy files likely relevant for H019:
...
Indicator file relevant to current issue:
...
H018 stop-state diagnostic, latest code:

- `C:\Users\equin\Documents\institutional-ea\quantcore\backtest\h018_stop_state_diagnostic.py`
- `C:\Users\equin\Documents\institutional-ea\scripts\diagnose_h018_stop_state_real.py`
- `C:\Users\equin\Documents\institutional-ea\tests\test_h018_stop_state_diagnostic.py`
...
- `docs/operations/handoffs/HANDOFF_51.md`
...
Expanded broker-native USDJPY and XAUUSD M1/H4 data is conditionally accepted for H017/H018/H019-style strict validation only under strict complete-window restrictions.
...
HistData remains rejected for H017/H018/H019 validation under current evidence.
...
### 1. Strict H018 Broker-Native Validation Was Explicitly Authorized And Run Earlier
...
Full test result after that code:
...
Directly count how often H017 holds a Donchian signal while the same-side raw Chandelier stop is already breached/non-protective at decision close.

This diagnostic is read-only with respect to strategy semantics.
...
- This strongly supports the stale-held-signal / stop-lifecycle mismatch diagnosis.
- The invalid-stop issue is overwhelmingly caused by H017 continuing to emit a held Donchian direction after the same-side raw Chandelier panel is no longer protective.
- This does not validate H018.
- This does not promote H017.
- This does not approve switching panels.
- This does not approve live trading.

First breached same-side stop observation from latest diagnostic:
...
- entry_time: `2021-07-05 21:00:00+00:00`
- signal_value: `-1.0`
- previous_signal_value: `-1.0`
- signal_state: `held_continuation`
- decision_close: `1791.224`
- stop_price: `1786.1805360200765`
- selected_stop_panel: `stops_short`
- same_side_stop_valid_at_decision_close: `False`
...
Confirmed from code inspection and diagnostics:
...
The latest stop-state diagnostic confirms this diagnosis:

- `1544` same-side stops breached at decision close.
- `1543` of those were held continuations.
- Only `1` was a fresh signal.

## Important Interpretation Guardrail

The evidence does not mean “fix the guard.”
...
## Recommended Next Phase: H019

The user wants to hand off here and then start H019.

H019 should not be a silent H017 patch.

H019 should explicitly define strategy/stop lifecycle semantics before code.

H019 should be treated as a new hypothesis identity if behavior changes.

Most likely H019 options:

### Option 1: Stateful Chandelier Exit Semantics

Proposed meaning:

- Donchian breakout enters direction.
- Same-side Chandelier breach exits or flatlines.
- Strategy remains flat until a fresh Donchian trigger.
- The Chandelier stop becomes part of a real active-position lifecycle, not just a raw indicator panel.
- This likely means H019 is not H017; it is a new hypothesis.

Possible long lifecycle:

- Fresh long Donchian breakout creates long state.
- While long:
  - use long Chandelier stop as protective reference.
  - if close or intrabar low breaches the long stop under the chosen semantic, flatten.
  - remain flat until a new Donchian trigger occurs.

Possible short lifecycle:

- Fresh short Donchian breakout creates short state.
- While short:
  - use short Chandelier stop as protective reference.
  - if close or intrabar high breaches the short stop under the chosen semantic, flatten.
  - remain flat until a new Donchian trigger occurs.

Key unresolved design choices for H019 option 1:

- Does stop-state flattening happen on H4 close only, or intrabar using M1?
- If using M1, does this belong in strategy signal generation or event execution?
- After stop-out, can the same Donchian direction re-enter without a new breakout?
- Does a same-direction breakout after flat count as a fresh trigger if prior direction is already same?
- How should stops be trailed statefully?
- How should position sizing reference the active stateful stop?
- Is H019 allowed to use the same H017 parameters initially, or should this be a new locked config with explicit rationale?

Recommendation if choosing option 1:

- Start with a small design note or in-chat decision.
- Then implement a pure, testable state machine before touching real-data validation.
- Use synthetic unit tests first.
- Do not run real-data validation until explicitly authorized.

### Option 2: Continuous Donchian Exposure With Separate Always-Valid Protective Stop

Proposed meaning:

- Donchian held direction remains desired continuous exposure.
- Chandelier panels are not the protective stop.
- A separate protective stop construction is defined so every event trade has valid stop geometry.
- Sizing semantics must be redesigned around the new stop.

Risks:

- This is a larger semantic change than option 1.
- It can accidentally create arbitrary stops just to satisfy guards.
- It could hide that the original Chandelier exit thesis failed.
- It requires very careful risk and cost modeling.

If choosing option 2, do not implement until:

- protective stop definition is explicit
- sizing reference is explicit
- invalid-stop guard remains strict
- minimum stop distance and leverage guards remain strict

### Option 3: Reject H017/H018 Line

Current evidence is already sufficient to reject the present strategy/execution contract.

This would mean:

- H017 failed.
- H018 guard diagnostics explained why.
- H019 might start from a cleaner strategy design instead of repairing H017.

## Recommendation For H019

Recommended starting point:

- Choose option 1 in plain English:
  - H019 = Donchian entry + stateful Chandelier exit/flatline lifecycle.
- Do not code until the exact lifecycle semantics are stated.
- Treat H019 as a new hypothesis identity.
- Preserve H017 as failed historical evidence.
- Keep all H018 guards.

Recommended first concrete H019 task:

- Write a minimal H019 design decision in either chat or a short doc.
- Then create a small state-machine module with synthetic tests.
- Do not touch real-data validation yet.

Possible H019 first module if authorized later:

- `quantcore/strategy/h019.py`

Possible H019 first test file if authorized later:

- `tests/test_h019.py`

But do not create those until H019 semantics are explicitly chosen.
...
- Do not live trade.
- Do not use HistData for H017, H018, or H019 validation.
...
- Do not allow code-test count to drop below `577` without explicit test-removal phase.
...
Understood. Continuing after HANDOFF_51.

I understand:

- Windows / PowerShell / VS Code / Python 3.12.10 in `.venv`.
- Current branch should be `main`.
- Latest expected commit should be `Add handoff document #51 before H019`.
- Previous expected code commit should be `79a464f Add H018 stop-state diagnostic`.
- Current full-test anchor is `577 passed`.
...
- HistData is rejected for H017/H018/H019 validation.
...
- Stop-state diagnostic found 1544 same-side stops breached at decision close, with 1543 of those being held continuations.
- This confirms the stale-held-signal / stop-lifecycle mismatch.
- This does not approve switching panels.
- H019 should begin by explicitly defining new strategy/stop lifecycle semantics.
- No code yet.
...
After hygiene passes, I will help define H019 semantics before any code.