...
If any older handoff conflicts with this file, this HANDOFF_73 wins.
...
Current full-test anchor after latest code work: **867 passed**.
If full test count drops below 867 without planned test removal, treat as a regression.
...
Latest full suite after WOULD_OPEN synthetic validation code work:

867 passed in 15.76s

If full tests pass but count drops below 867 without explicit test-removal intent, stop and treat it as a regression. Check for untracked experimental test files polluting the suite.
...
USDJPY: spread_price 0.01, commission_usd_per_lot_per_fill 7.0, stop_slippage_atr_fraction 0.05
XAUUSD: spread_price 0.30, commission_usd_per_lot_per_fill 10.0, stop_slippage_atr_fraction 0.05
...
Implemented in: quantcore\backtest\h017_event.py
...
Hard max 10.0x
< 10.0 passes, == 10.0 passes, > 10.0 fails closed
...
slow_window = 5, slope_lag = 2, atr_window = 3, pullback_window = 3
min_pullback_atr = 0.25, max_pullback_atr = 3.0, min_slope_atr = 0.05
...
Does not replace full execution timestamp shuffle. Do not run brute-force timestamp shuffle again until redesigned.
...
USDJPYm, XAUUSDm
Observed order facts: trade_mode: 4, execution_mode: 2, order_filling_modes: 3, order_modes: 127
This does not test actual order placement, rejection behavior, requotes, slippage.
...
Forbidden in dry-run/log-only: OrderSend, OrderSendAsync, OrderCheck, CTrade, #include <Trade...>, PositionOpen, PositionClose, PositionModify, pending order helpers, MqlTradeRequest, MqlTradeResult.
Dry-run output only: WOULD_OPEN, NO_ACTION, BLOCKED.

Dry-run action export result:
Rows: 5166. WOULD_OPEN: 459, NO_ACTION: 4707, BLOCKED: 0, Violations: 0, Verdict: PASS.
This does not approve demo/live/Phase 4.

## MT5 Terminal/Account Preflight

Python terminal/account preflight passed.
Observed account/terminal facts: Account currency: USD, Account leverage: 2000, Terminal connected: true, Terminal trade_allowed: false.
Important: terminal_trade_allowed=false was observed and must be understood before later execution gates. This does not block read-only preflight. This does not approve demo trading.

## EA Runtime State

Current EA source: ea_mt5\Experts\H024_LogOnly_Preflight.mq5
Current runtime schema: schema_version = h024_ea_log_only_preflight_v2
Current EA runtime input version: InpEaVersion = 0.6
Current MQL5 property version: #property version "0.600"

## Work Since HANDOFF_72

1. Cleaned up untracked experimental test files (h024_full_h4_runtime_harness*.py) that were incorrectly polluting the test suite and causing false regressions. Restored clean test anchor at 867 passed.
2. Added 	ests/test_h024_ea_strategy_intent_detail_static.py to enforce static guardrails, ensuring H024StrategyIntentDetail() contains no execution surface (no OrderSend, 	icket=, lots=, etc.) and formats strings strictly.
3. Added 	ests/test_h024_ea_strategy_intent_decision_harness.py to provide a pure-Python mock of the intended MQL5 conditional logic, proving warmup gates and conflict blocking.
4. Implemented 	ests/h024_full_h4_runtime_ci_harness.py, a synthetic CI harness that reads historical Exness MT5 H4 rates and proves the WOULD_OPEN logic correctly triggers and isolates strictly to diagnostic intents without requiring MT5 execution APIs.
5. Recorded docs/operations/H024_EA_WOULD_OPEN_SYNTHETIC_VALIDATION_RESULT.md confirming PASS.

## Current Deployment Reality Check

Answer if asked whether ready to demo/live: No.
H024 is still not demo deployable.

What is now true:
- Backtest H024 is promising. Broker-cost reconciliation passed. Static MT5 order-behavior facts passed.
- MT5 terminal/account read-only preflight passed. MQL5 log-only EA shell exists.
...
- **NEW:** Synthetic WOULD_OPEN validation passed. The WOULD_OPEN semantic is successfully isolated from actual order execution. Conflict blocks and warmup gates are proven.
...
- Runtime WOULD_OPEN row emission has not been observed under a real, live signal condition (requires waiting for the market).
- 2023 weakness remains real and visible.

## Recommended Next Work

Best next gate: Apply Test-Driven Development (TDD) to **Position Sizing math**.

1. **Python Scaffolding First:** Add Python static guards to ensure the future MQL5 sizing function will contain no execution APIs (OrderSend, CTrade, etc.).
2. **Pure Math Mock:** Build Python reference tests for the H020 sizing contract (account balance, risk %, stop distance, tick size/value) to define expected MQL5 output.
3. **MQL5 Implementation:** Write the pure-math sizing function in H024_LogOnly_Preflight.mq5.

## Current Safe Automation State

Already automated:
Copy EA source to terminal MQL5\Experts. Compile EA with MetaEditor. Reset/Collect/Verify runtime CSV. Terminal Preflight. Profile/Template validation.

Not approved:
EA chart attachment/detachment automation. GUI automation. MT5 launcher/profile mutation affecting live terminals. Order-send automation. Demo/live execution adapter.

## Absolute Do-Not Rules

Do not demo trade, live trade, or approve Phase 4.
Do not treat H024 as deployment-ready.
Do not hide 2023 weakness with a filter or time/session buckets.
...
Do not allow full-test count to drop below **867** without explicit test-removal intent. Do not use untracked test files to dictate regressions.
Do not add OrderSend, OrderCheck, CTrade, #include <Trade...>, position open/close code, or MqlTradeRequest.

## Known Repo Hygiene Lessons

Do not repeat unrooted data/ in .gitignore.
PowerShell does not support Linux heredocs.
Treat code test-count drops as regressions. Untracked test files will pollute pytest; ensure git clean or explicit tracking.
Network/DNS push failures can happen; stop development until git push succeeds.
MetaEditor CLI may return nonzero even when compile succeeds with warnings; verify .ex5 refresh. Treat missing .ex5 as compile failure.
Python indentation inside PowerShell here-strings caused repeated IndentationError; use simpler PowerShell-native file append.
Resolve-Path fails for files that do not exist yet; use [System.IO.Path]::GetFullPath(tests\test_h024_ea_preflight_log_verifier.py).

## Exact First Response The Next AI Should Give

Understood. Continuing from HANDOFF_73.

I understand:

Windows / PowerShell / VS Code / Python 3.12.10 in .venv.
Branch should be main, clean except possibly local reports/, and pushed.
Current full-test anchor is 867 passed.
...
Next target is strict TDD for pure-math Position Sizing.
