# Project Handoff — Institutional-Grade MT5 EA on Retail Stack (Continuation #4)

You are continuing an existing project that has already gone through FOUR AI handoffs. Read this entire document before responding. Do not invent context that isn't here. When in doubt, ask the user before writing code.
...
- Phase 1 — Research Framework (`quantcore` package). IN PROGRESS — at sub-phase 1.10.
...
├── README.md
├── HANDOFF_3.md               (previous handoff, kept for history)
├── HANDOFF_4.md               (this document)
...
1.10 Probabilistic Sharpe + Deflated Sharpe                                   ⬅️ NEXT
...
**Test count after Phase 1.9: 155 passing.** Anchor breakdown: loaders 9 + reconcile 9 + outliers 10 + smoke 3 + checksums 14 + lookahead 15 + metrics 26 + purged_kfold 20 + cpcv 24 + walk_forward 25 = 155. The pytest output `155 passed` is the source of truth — do not regress below this.
...
- WalkForward exposes `mode: Literal["rolling", "anchored"]`, `train_size`, `test_size`, `step_size` (defaults to `test_size`), `embargo_pct`, `t1`. One class, two modes — DO NOT split it. Returns a `WalkForwardSplit` frozen dataclass with `fold_id`, `train_idx`, `test_idx`, `train_start`, `train_end`, `test_start`, `test_end`.
- **TZ-aware `t1` trap (1.9 lesson):** `pd.Series.values` on a tz-aware datetime Series strips the timezone. Use `series.iloc[idx]` and `.to_numpy()` AFTER the comparison, not `.values` before. See `walk_forward.py` purge block.
...
2. Continue from sub-phase 1.10 (Probabilistic Sharpe + Deflated Sharpe).
...
9. Pre-existing Pylance/static-type warnings (deferred to a future type-cleanup sub-phase, do NOT fix in-line):
...
- All six `quantcore/validation/` files tracked: `__init__.py`, `cpcv.py`, `lookahead.py`, `metrics.py`, `purged_kfold.py`, `walk_forward.py`.
...
- Most recent commits at handoff time (newest first):
  - Phase 1.9: walk-forward CV with rolling and anchored modes, embargo and tz-aware t1 purge
  - Add handoff document #3 for AI continuity at Phase 1.9
...
- User is ready for Phase 1.10 — Probabilistic Sharpe + Deflated Sharpe.
...
- 155 tests passing, next deliverable is Phase 1.10 — Probabilistic Sharpe Ratio (PSR) and Deflated Sharpe Ratio (DSR).
...
- DO NOT produce Phase 1.10 code yet — wait for the user to type "continue" or ✅ AND wait for the hygiene-verification output (§10).

Mandatory hygiene-verification step BEFORE producing Phase 1.10 code: ask the user to paste the output of:
...
Confirm: all expected files per §3 are tracked, working tree clean, origin/main in sync, all six `quantcore/validation/` files present (including `walk_forward.py`). If anything is wrong, fix the hygiene first; do not proceed.

Optional design questions to ask before writing Phase 1.10 code:

1. **Module name.** Suggested: `quantcore/validation/deflated_sharpe.py` containing both PSR and DSR (they share the same skew/kurtosis-adjusted Sharpe variance machinery). Alternative: separate `psr.py` and `dsr.py`. Default if user says "you decide": single file `deflated_sharpe.py`.

2. **API shape.** Suggested:
   - `probabilistic_sharpe_ratio(returns: np.ndarray | pd.Series, sr_benchmark: float = 0.0, periods_per_year: int = 252) -> PSRResult`
   - `deflated_sharpe_ratio(returns: np.ndarray | pd.Series, sr_estimates: np.ndarray, sr_benchmark: float = 0.0, periods_per_year: int = 252) -> DSRResult`
   - `PSRResult(psr: float, observed_sr: float, skew: float, kurtosis: float, n: int, sr_benchmark: float)` — frozen dataclass.
   - `DSRResult(dsr: float, observed_sr: float, skew: float, kurtosis: float, n: int, expected_max_sr: float, n_trials: int, sr_benchmark_deflated: float)` — frozen dataclass.
   - Both return probabilities in `[0, 1]`. Reuse `sharpe_ratio` from `quantcore.validation.metrics` for the observed SR computation to avoid drift.

3. **Annualization.** PSR/DSR formulas use the *non-annualized* (per-period) Sharpe. Internally, compute per-period SR and per-period skew/kurtosis; expose `periods_per_year` only for display purposes (i.e. `PSRResult.observed_sr` should be the annualized value to match how `metrics.sharpe_ratio` already reports it, but the PSR probability itself is computed on per-period). Default if user says "you decide": follow this convention.

4. **References to cite in docstrings.**
   ...
   - Mention these in the module docstring and the function docstrings.
...
## 13. Phase 1.9 Specific Notes (for the new AI's reference)

- `WalkForward` is in `quantcore/validation/walk_forward.py`. One class, `mode="rolling"|"anchored"`. The rolling mode slides a fixed-size train window forward by `step_size` (default = `test_size`); the anchored mode keeps train_start at 0 and grows the train window as folds advance.
- Embargo gap = `int(embargo_pct * n)` bars inserted between train_end and test_start.
- `t1` purge uses `self.t1.iloc[train_idx]` (NOT `.values`) to preserve tz-awareness, with comparison `train_t1 < test_start_ts` and `.to_numpy()` afterwards. Strict less-than means a label ending exactly at `test_start` is purged.
- Folds where `t1` purges all training samples are SKIPPED (offset advances, no degenerate empty-train split is yielded).
- `get_n_splits()` counts by iteration (not closed-form) because skipped folds make the closed form wrong.
- Test file `tests/test_walk_forward.py`, 25 tests, covers: rolling basics, anchored basics, embargo gap, t1 purge (overlap, no-overlap, all-purged-skip), input validation (mode, sizes, embargo bounds, empty index, non-monotonic index), structural API (fold_id sequential, get_n_splits parity, timestamp/index correspondence).