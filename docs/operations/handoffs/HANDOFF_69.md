...
If any older handoff conflicts with this file, this HANDOFF_69 wins.
...
HANDOFF_68 - After Timer-Driven H024 EA Log-Only Intent Runtime Preflight
...
Current full-test anchor after latest code work: 839 passed.
If full test count drops below 839 without planned test removal, treat as a regression.
...
a625554 Add handoff document #68
a51dc8f Harden H024 MT5 preflight helper compile handling
215b6b7 Add H024 MT5 log-only preflight local helper
eac6d06 Add timer-driven H024 EA intent preflight
8d40445 Add H024 EA log-only intent rows
a8c3c63 Add H024 EA source static verifier
0be76e8 Add handoff document #67
...
839 passed in 15.69s

If full tests pass but count drops below 839 without explicit test-removal intent, stop and treat it as a regression.
...
reports\h024_mt5_terminal_preflight.json
H024 Log-Only EA Runtime Evolution
Initial Log-Only Skeleton
...
USDJPYm
XAUUSDm
H024 EA Log-Only Runtime Preflight Result From HANDOFF_67
...
The previous runtime result did not prove OnTick or intent logging.
It only showed INIT and DEINIT.
It did not approve demo trading, live trading, Phase 4 execution, order placement, order modification, or order closing.
The observed terminal_trade_allowed=false condition remained important.
Work Since HANDOFF_68
Standalone MQL5 Source Static Verifier
...
Verify H024 log-only MQL5 EA source has no execution surface from a standalone script, not only pytest static checks.
Preserve no-order-send boundary explicitly.

Forbidden execution-surface patterns include:
...
PositionClose
PositionModify
buy/sell helper calls
pending-order helper calls
OrderDelete
OrderModify
TRADE_ACTION_DEAL
...
Add explicit INTENT event rows from EA runtime.
Keep the EA preflight stage no-order.
Constrain allowed runtime intent action for this stage to NO_ACTION:*.

Details:

Added WriteIntentRow().
OnTick() writes:
TICK
INTENT
...
NO_ACTION:kill_switch_blocked when kill switch is blocked
NO_ACTION:log_only_unblocked otherwise

Runtime verifier allows event set:
...
Runtime verifier rejects WOULD_OPEN intent rows for this specific preflight EA stage.
Runtime verifier rejects unknown intent actions.

Important limitation discovered:

On Saturday / market closed, no ticks occurred.
Runtime file showed only INIT and DEINIT, so OnTick()-based INTENT was not validated.
Timer-Driven H024 EA Intent Preflight
...
Avoid waiting for market open just to validate log-only INTENT runtime path.
Emit INTENT rows from timer, not only from market ticks.
Preserve no-order boundary.
...
OnInit() now:
opens log file
calls EventSetTimer(InpTimerSeconds)
writes INIT
writes immediate INTENT
Added:
void OnTimer()
{
   WriteIntentRow();
...
OnDeinit() now calls:
EventKillTimer();
OnTick() still writes TICK and INTENT when market ticks exist.
H024 MT5 Log-Only Preflight Local Helper
...
Automate the safe parts of MT5 EA handling from VS Code / PowerShell / Python:

copy repo EA source to MT5 terminal MQL5\Experts
optionally compile with MetaEditor64.exe
optionally collect runtime CSV from MT5 terminal MQL5\Files
verify collected CSV with existing runtime verifier
...
Example helper paths discovered on this machine:

MetaEditor:

C:\Program Files\MetaTrader 5\MetaEditor64.exe

MT5 terminal data directory:

C:\Users\equin\AppData\Roaming\MetaQuotes\Terminal\D0E8209F77C8CF37AD8BF550E51FF075
Hardened Compile Handling
...
MetaEditor CLI returned process code 1.
But logs\metaeditor.log showed successful compile:
Compile ... H024_LogOnly_Preflight.mq5 - 0 errors, 1 warnings
.ex5 was refreshed.
Therefore the helper was too strict if it treated nonzero process return as a hard failure.

Fix:

Added CompileOutcome.
Accept compile if:
return code is 0, or
.ex5 was refreshed.
Added .ex5 freshness detection.
Added --reset-runtime-log option to delete stale terminal runtime CSV before manual EA attachment.
Latest Timer-Driven Runtime Preflight Result From HANDOFF_68

Performed after commit:

a51dc8f Harden H024 MT5 preflight helper compile handling

Command used to copy/compile/reset runtime log:
...
Then user manually attached the EA to:
...
Each was left attached for roughly a few seconds and removed.

Collection/verification command:

python .\scripts\run_h024_mt5_log_only_preflight_local.py `
  --terminal-data-dir "C:\Users\equin\AppData\Roaming\MetaQuotes\Terminal\D0E8209F77C8CF37AD8BF550E51FF075" `
  --collect

Import-Csv reports\h024_ea_log_only_preflight.csv |
  Group-Object symbol,event |
...
Timer-driven log-only intent works even while the market is closed.
The EA runtime emits explicit INTENT rows without needing ticks.
The verifier accepted the fresh runtime file.
Both required H024 broker symbols were covered.
Kill switch remained blocked.
There were no verifier violations.
Still no TICK rows because the market was closed, but this no longer blocks intent-path validation.
Still no demo/live/Phase 4 approval.
Still no order path.

Local runtime CSV remains uncommitted:

reports\h024_ea_log_only_preflight.csv

Do not commit this CSV.

New Work Since HANDOFF_68
Versioned H024 EA Runtime Preflight Schema
...
823 passed in 15.65s

What changed:

EA runtime CSV now has explicit schema/source/runtime metadata.
Verifier now rejects stale unversioned logs.
Helper/verifier tests updated to v2 schema.
Test anchor advanced from 821 to 823.
...
After commit e043de5, the log-only EA was copied, compiled, runtime log was reset, manually attached to both required broker symbols, removed, collected, and verified.
...
H024 EA v3 Market-State Preflight Rows
...
825 passed in 16.98s

What changed:

EA version advanced to 0.3.
Runtime verifier expects EA 0.3.
Added new event:
MARKET_STATE
EA logs H4 and M1 observation detail from MT5 runtime context.
Still no execution surface; static verifier passed.
Still no demo/live/Phase 4 approval.

Market-state detail format:

H4:time=<server time>;open=<price>;high=<price>;low=<price>;close=<price>;tick_volume=<volume>|M1:time=<server time>;open=<price>;high=<price>;low=<price>;close=<price>;tick_volume=<volume>

Unavailable data is logged as:

H4:unavailable
M1:unavailable

Verifier requires MARKET_STATE detail to include both H4: and M1:.

H024 EA v3 Market-State Runtime Result
...
After commit e611d62, the log-only EA was copied, compiled, runtime log was reset, manually attached to both required broker symbols, removed, collected, and verified.
...
H024 MT5 Automation Target Preflight
...
828 passed in 17.06s

What changed:

Added --automation-target-preflight.
Added explicit rejection for --attach-detach.
Validates:
terminal data dir
MQL5\Experts
MQL5\Files
MetaEditor path
EA source static safety
expected schema version
EA version
runtime mode
kill-switch default
symbols
Still no GUI automation.
Still no attach/detach.
Still no order surface.

Observed local automation target preflight:
...
The helper then copied and compiled the EA safely:
...
H024 MT5 Profile Template Plan Validator
...
Add inert JSON plan validator for future MT5 profile/template automation.
This is read-only and does not launch MT5, attach/detach EAs, automate GUI actions, or place/modify/close orders.

Validator requires:

exact terminal data dir shape
expected Exness demo account/server metadata
required charts for USDJPYm and XAUUSDm
H024_LogOnly_Preflight
schema h024_ea_log_only_preflight_v2
EA 0.3
runtime mode log_only_preflight
kill switch blocked
attach_detach_enabled = false
gui_automation_enabled = false
order_send_enabled = false

Expected constants in validator:
...
The validator has tests for:

valid plan payload
valid JSON plan file
missing file
invalid JSON
wrong terminal shape
wrong account server
attach_detach_enabled = true
order_send_enabled = true
wrong symbols
unblocked kill switch input
wrong EA version input
...
Current runtime INTENT rows are preflight-only NO_ACTION:*, not strategy-derived WOULD_OPEN.
Current runtime MARKET_STATE rows only confirm H4/M1 data access; they do not compute H024 signals.
No runtime H024 bar/state ingestion exists inside EA.
No runtime intended-action logs from actual H024 signal/state machine exist inside EA.
...
Add richer no-order runtime observation logging.
Add runtime H024 bar/state observation logs.
Add verifiers for runtime no-order intended-action logs.
Add stricter log format/versioning if needed.
Automate safe copy/compile/collect/verify steps from VS Code/PowerShell/Python.
Validate inert MT5 profile/template automation plans before any launcher work.
...
GUI attach/detach automation without a separate approved safety design.
Answer To User's Automation Question

The user asked whether MT5 can be fully automated: copy, compile, attach, detach, everything.

Current answer:

Copy/compile/reset/collect/verify is already automated.
Target preflight is now automated.
Inert profile/template plan validation is now available.
Attach/detach is not approved yet.
Blind GUI automation should not be used.
Profile/template-driven automation might be possible later, but only after strict file/terminal/account/chart validation and an explicit safety gate.
Order automation remains out of scope.

Reasoning:

Full GUI automation can accidentally interact with the wrong terminal, chart, account, or EA input.
MT5 profile/template file manipulation can be brittle and terminal-specific.
Before any launcher/attach automation, the project must prove it can validate the intended target and reject unsafe plans.

Current safe next direction:

Use scripts\validate_h024_mt5_profile_template_plan.py as the schema/validator foundation.
Add a sample inert plan under docs or fixtures if needed.
Add profile/template file inspection only after understanding MT5 file formats on this exact terminal.
Do not launch MT5 automatically yet.
Do not attach/detach automatically yet.
Do not add order-send.
Recommended Next Work

Best next gate:

Add an inert sample H024 MT5 profile/template automation plan and maybe a command example in docs or tests.

Purpose:

Show exactly what a valid automation plan looks like.
Make future automation conversations concrete.
Keep it inert and fail-closed.
Avoid guessing when later implementing profile/template generation.

Possible file:

docs/operations/H024_MT5_PROFILE_TEMPLATE_AUTOMATION_PLAN_EXAMPLE.json

But be careful with machine-specific paths. If committed, use placeholder terminal paths that intentionally do not pass local validation, or put actual local path only in docs text and do not treat it as reusable config.

Alternative next gate:

Add a local-only report generator for a plan JSON under reports/, not committed.

Example:

reports\h024_mt5_profile_template_plan.local.json

Do not commit this local report.

The generator could use known local values:
...
ea_name = H024_LogOnly_Preflight
schema_version = h024_ea_log_only_preflight_v2
ea_version = 0.3
runtime_mode = log_only_preflight
...
python .\scripts\validate_h024_mt5_profile_template_plan.py reports\h024_mt5_profile_template_plan.local.json

After that, consider adding profile/template inspection tooling only if needed.

Another useful next gate:

Add runtime BAR_OBSERVATION rows with closed-bar identification.

Purpose:

Current MARKET_STATE reads current H4/M1 bars from MT5.
H024 strategy decisions must eventually use completed H4 bars, not arbitrary current forming bars.
A no-order runtime observer can log:
latest complete H4 bar time
latest complete H4 OHLC
latest M1 bar availability around that time
server time vs bar time
This would move closer to runtime H024 state ingestion without generating WOULD_OPEN.

Do not jump directly to actual WOULD_OPEN strategy intent inside EA until closed-bar observation semantics are cleaner.
...
allow full-test count to drop below 839 without explicit test-removal intent
...
add GUI attach/detach automation without an explicit safety design and approval
...
git diff --check caught a blank line at EOF in a test file; fix whitespace before committing.
Exact First Response The Next AI Should Give
Understood. Continuing from HANDOFF_69.

I understand:

- Windows / PowerShell / VS Code / Python 3.12.10 in .venv.
- Branch should be main, clean except possibly local reports/, and pushed.
- Current full-test anchor is 839 passed.
...
- Standalone H024 EA source static verifier exists and blocks MQL5 execution-surface tokens.
- Log-only MQL5 EA emits explicit timer-driven INTENT rows.
- EA runtime CSV schema is versioned: h024_ea_log_only_preflight_v2.
- Current EA version is 0.3.
- Fresh EA 0.3 terminal runtime evidence produced 22 rows, 0 violations, both USDJPYm and XAUUSDm covered.
- Runtime MARKET_STATE rows confirmed H4/M1 data access from EA context for both symbols.
- Local MT5 helper copies/compiles/resets/collects/verifies the log-only EA preflight path.
- MetaEditor nonzero return can be accepted if EX5 was refreshed.
- Automation target preflight exists and passed locally.
- Inert MT5 profile/template plan validator exists.
...
- The next recommended gate is either an inert local profile/template plan generator/validator or closed-bar BAR_OBSERVATION rows before any runtime H024 signal intent.
