HANDOFF 42 - After H018 Minimum Stop-Distance Policy Acceptance And Implementation

This handoff is intentionally self-contained enough for a new AI to continue safely.

If any older handoff conflicts with this file, this HANDOFF_42 wins.
...
Latest expected commit before this HANDOFF_42 document is committed:

3ae9e18 Implement H018 minimum stop-distance guard

Expected latest commit after this handoff is committed:

Add handoff document #42 after H018 minimum stop-distance guard
...
The increase is expected because new focused synthetic tests were added.

Focused H017 event test anchor after the H018 minimum stop-distance guard:
...
New focused tests added in tests/test_h017_event.py:
...
- test_h018_minimum_stop_distance_at_or_above_one_spread_passes_guard with param cases:
  - XAUUSD buy at exactly 0.30 distance
  - XAUUSD sell at exactly 0.30 distance
  - XAUUSD buy above 0.30 distance
  - XAUUSD sell above 0.30 distance
  - USDJPY buy at exactly 0.01 distance
  - USDJPY sell at exactly 0.01 distance
...
A separate maximum notional/leverage guard is still required before any real-data validation attempt.
...
Before the new H018 minimum stop-distance guard, positive near-zero stop distances could create extreme exposure.

The new H018 minimum stop-distance guard blocks raw stop distances smaller than one modeled spread, but this is still not enough by itself to guarantee safe exposure.

Maximum notional/leverage protection remains open.
...
Strict event-driven backtest result before invalid-stop and H018 minimum-distance guards:
...
Fatal USDJPY fill before invalid-stop and H018 minimum-distance guards:
...
Fatal XAUUSD fill before invalid-stop and H018 minimum-distance guards:
...
Research verdict printed by the runner before invalid-stop and H018 minimum-distance guards:
...
Do not rerun broad strict real-data validation as a promotion attempt after the invalid-stop guard or after the H018 minimum-distance guard.
...
- H018 minimum stop-distance fail-closed guard implemented and tested.
...
3. Skip trade is rejected for first H018 validation-mode implementation.
4. Position clipping is rejected for first H018 validation-mode implementation.
5. Warn-and-continue is rejected.
6. Log-only continuation is rejected for validation mode.
7. Diagnostic-only continuation is deferred to a separately labeled diagnostic mode.
...
- No exposure measurement basis.
- No sizing reference final decision.
- No stop-validity reference final decision.
...
Work Completed Since HANDOFF_41

Commit 2c379fb - Accept H018 minimum stop-distance policy
...
- raw_stop_distance = abs(raw_h4_entry_open - stop_price)
- minimum_stop_distance = one modeled spread for the symbol
- USDJPY threshold = 0.01
- XAUUSD threshold = 0.30
- raw_stop_distance < threshold fails closed
- raw_stop_distance == threshold passes this guard
- raw_stop_distance > threshold passes this guard
- validation-mode violation policy is fail closed
- no skip
- no clipping
...
Commit 3ae9e18 - Implement H018 minimum stop-distance guard
...
- Imported math.
- Imported get_default_cost_spec from quantcore.backtest.cost_model.
- Added H018MinimumStopDistanceError.
- Added _validate_minimum_stop_distance(...).
- Called _validate_minimum_stop_distance(...) after _validate_directional_stop(...) and before size_position_from_risk(...).
- Guard uses get_default_cost_spec(symbol).spread_price as threshold.
- Guard fails closed if raw_stop_distance is below threshold and not effectively equal by math.isclose.
- Guard preserves equality-pass boundary for the minimum-distance rule.
...
After HANDOFF_42 hygiene, the next AI should not create another broad draft-only document by default.

Recommended next sub-phase:

Phase 3.26-cf - Decide and/or implement maximum notional/leverage guard, starting with API inspection and narrow policy acceptance.

Rationale:

- The minimum stop-distance guard blocks sub-spread denominators.
- It does not cap notional or leverage.
- A directionally valid stop exactly one spread away can still size very large.
- A maximum notional/leverage guard is still required before any real-data validation attempt.

Safe next steps:

1. Hygiene verification only.
2. Inspect docs/operations/H018_MAX_NOTIONAL_LEVERAGE_DECISION_RECORD.md.
3. Inspect quantcore/backtest/portfolio.py:
   - InstrumentSpec
   - PositionSize
   - get_default_instrument_spec
   - size_position_from_risk
4. Inspect tests/test_portfolio.py and tests/test_h017_event.py.
5. Decide a conservative maximum exposure policy before coding.
6. Do not choose thresholds casually.
7. If choosing a threshold, specify:
   - exact exposure basis,
   - exact threshold,
   - instrument handling,
   - boundary behavior,
...
8. Then implement focused synthetic tests before or with code.

Potential conservative maximum exposure policy to consider, but not yet accepted:

- maximum notional/equity ratio per trade, measured as position_size.notional_quote / equity_usd before costs.
- But for USDJPY, notional_quote is JPY not USD, so this basis may be misleading unless converted or explicitly named.
- Another option is actual risk plus notional converted to USD where possible.
- Must inspect current PositionSize fields and fill_pnl_usd conversion behavior before deciding.
- Do not implement until the decision record is accepted.

Alternative next sub-phase:

Phase 3.26-cf-alt - Inspect and repair documentation consistency after H018 minimum stop-distance implementation.

Reason:

Some H018 docs may still say minimum-distance guard is not implemented. If doing this, keep it narrow and do not create broad new docs.

Absolute Do-Not Rules At HANDOFF_42
...
- Do not call H018 minimum stop-distance implementation H018 validation.
- Do not call H018 governance plans H017 or H018 promotion.
- Do not rerun broad strict validation after the invalid-stop guard or minimum-distance guard unless explicitly authorized as diagnostics or later H018 validation.
...
- Do not treat positive near-zero stop-distance behavior as fully solved for account risk.
...
- Do not treat H018_DECISION_RECORD_INDEX.md as a full policy decision; it is an index, though it now references accepted fail-closed and minimum-distance policies.
...
- Do not treat accepted fail-closed policy or accepted minimum-distance policy as choosing maximum exposure limits.
- Do not treat accepted minimum-distance policy as authorizing real-data validation.
...
Understood. I am continuing after HANDOFF_42.

I understand:

- Windows / PowerShell / VS Code / Python 3.12.10 in .venv.
- Current full-test anchor is 545 passed.
- Focused H017 event test anchor is 23 passed.
- Latest expected commit after handoff commit is Add handoff document #42 after H018 minimum stop-distance guard.
- Previous commit before the handoff should be 3ae9e18 Implement H018 minimum stop-distance guard.
...
- This guard uses get_default_cost_spec(symbol).spread_price.
- This guard runs after directional stop validation and before sizing.
- This guard does not promote H017.
- This guard does not validate H018.
- This guard does not authorize a real-data rerun.
- Maximum notional/leverage behavior remains an open execution-semantics/account-risk issue.
...
- H018 minimum stop-distance decision record is Accepted and implemented.
...
- Recommended next logical sub-phase is maximum notional/leverage guard policy/API inspection, with no code changes first.
